...
Evaluations run at every checkpoint threshold during training (`n_chkpts` evenly spaced thresholds, the last landing at the conclusion of training), each persisting under `artifacts/<campaign>/settings/<setting>/<dataset>/<seed>/evals/eval<k>/` (`k` = 1..`n_chkpts`, `eval<n_chkpts>` being the final eval). Each eval writes one metrics file per eval group (`{native,native_macro,joint,joint_macro}.json`), and — when `dev.viz_manifold` is on (the manifold-viz master switch, see below) — caches its **raw** t-SNE/PCA projections to `projections.npz` and renders its plots to `viz/`; `evals/base/` (materialized from the base-eval cache) opens the sequence.

**Checkpoint selection.** Per eval group and per selection criterion, a trial's best checkpoint is the trained eval (`eval1`..`eval<n_chkpts>`; base is not a candidate) with the highest criterion score — `map` selects on `scores.comp.map.all`, `acc` on `scores.comp.acc.i2t` — earliest wins ties. The `eval_group` param (train.yaml, per-setting overridable) picks which group's best checkpoints have their **model weights** captured: whenever that group sets a new best under a criterion at eval time, the weights are saved to `chkpts/best/{map,acc}/model.pt` (same format as `chkpts/final/model.pt`, which is still written at trial end; the two may hold the same checkpoint), and `trial_metadata.json`'s `best_chkpt` records where each was captured as `{"map"/"acc": "<k>/<n_chkpts> (<samples-seen>M/<sample-volume>M)"}`. At trial end each criterion's per-group best-checkpoint metrics files are copied to `evals/_best/{map,acc}/{native,native_macro,joint,joint_macro}.json` (each file's `chkpt` field records its source checkpoint) — **all reporting** aggregates these best-checkpoint scores, not the final eval's, and their presence is the trial-completion signal for aggregation. Per criterion: the per-dataset `stats/{map,acc}/` jsons (+ their `listview/` subdirs) and the `stats/metrics/{map,acc}/<group>.xlsx` workbooks — each workbook keeps both its mAP and accuracy sheets, all sourced from its own criterion's checkpoints (e.g. the `map/` workbooks' accuracy sheet holds the acc scores at the best-mAP checkpoint), with the banner naming the selection (e.g. `bc_dev - dev3 (Joint-Macro; mAP-selection)`); the `stats/<dataset>/map/<group>.png` tables source the mAP-selected checkpoints and `acc/<group>.png` the acc-selected ones.
