...
**Note:** A campaign's matrix is **additive across runs**. After a campaign has run, you may **add** arms or coords (new members within an existing `ablation_arms` / `hpo_coords` combo group), `datasets`, or seeds (by raising `n_trials_screen`) and relaunch to extend it — already-completed trials are skipped and only the new ones run. (Adding a whole new *combo group* to a list re-joins every name in it, e.g. `hp` → `hp_2k`, so it reads as removing all prior arms/coords — start a new campaign for that.) You may **never remove** an arm, coord, dataset, or seed that a prior run recorded: the planned matrix and seeds are saved to each phase's `phase_metadata.json` at each launch, and a relaunch whose config drops any previously-recorded item raises an error before any trials execute (removing one would orphan its already-computed trials). To drop items, start a new campaign instead.
