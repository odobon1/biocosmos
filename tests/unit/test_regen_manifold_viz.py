...
def test_render_trial_copies_viz_to_selected_and_best(tmp_path, monkeypatch) -> None:
    ...
    (tmp_path / "trial_metadata.json").write_text(json.dumps({"chkpt": {"sel": 2, "best": 1}}))

    rmv.render_trial(tmp_path, cfg_manifold_viz=cfg)

    for sub in ("vanilla/8panel/joint.png", "cache/projections.npz"):
        assert (dpath_evals / "sel" / "viz" / sub).read_text() == "eval2"
        assert (dpath_evals / "best" / "viz" / sub).read_text() == "eval1"
...
def test_render_trial_leaves_unselected_trials_alone(tmp_path, monkeypatch) -> None:
    ...
    fpath.parent.mkdir(parents=True)
    fpath.write_text("x")
    (tmp_path / "trial_metadata.json").write_text(json.dumps({"chkpt": {"sel": None, "best": None}}))

    rmv.render_trial(tmp_path, cfg_manifold_viz={"pooled": {"enabled": False}})

    assert not (tmp_path / "evals" / "sel").exists() and not (tmp_path / "evals" / "best").exists()
