...
from utils.config import CampaignConfig
...
def _set_camp(monkeypatch, **spec) -> None:
    """Point the runner's camp-yaml read (_load_campaign_config) at `spec` -- what config/camps/<name>.yaml would
    hold (CampaignConfig's fields; suffix null). The runner re-reads the yaml before every trial, so calling this
    again -- between launches, or from inside a fake trial subprocess -- edits the campaign the way saving the
    file would."""
    cfg = CampaignConfig(suffix=None, **spec)
    monkeypatch.setattr(cr, "_load_campaign_config", lambda name: cfg)
...
def test_run_campaign_matrix(tmp_path, monkeypatch) -> None:
    ...
    def _fake_run_trial_subprocess(cfg_dict: dict, spare_render_pid=None):
        scheduled.append((cfg_dict["seed"], cfg_dict["dataset"], cfg_dict["arm"], cfg_dict["coord"],
                          cfg_dict["loss1"]["targ"], cfg_dict["opt"]["lr"]["init"]))
    ...
    meta = json.loads((tmp_path / "cmp_b" / "_screen" / "phase_metadata.json").read_text())
    assert meta["matrix"] == {d: {a: ["LR-2.0e-5", "LR-2.0e-6"] for a in ("sp", "hp")} for d in ("cub", "lepid")}
...
def test_run_campaign_raises_on_arm_coord_key_collision_before_side_effects(tmp_path, monkeypatch) -> None:
    ...
    with pytest.raises(ValueError, match=r"loss1\.targ.*appear in both ablation_arms and hpo_coords"):
        _set_camp(
            monkeypatch,
            n_trials_screen=1, n_trials_qual=None, trainval=False,
            datasets=("cub",),
            ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]],
            hpo_coords=[[{"loss1.targ": "mp", "name": "mp"}]],
        )
        cr.run_campaign("cmp_collide", "camp")
...
def test_run_campaign_writes_split_overrides(tmp_path, monkeypatch) -> None:
    ...
    cr.run_campaign("cmp_c", "camp")

    fpath = tmp_path / "cmp_c" / "_screen" / "_datasets" / "cub" / "_arms" / "sp" / "_coords" / "lr" / "overrides.json"
    assert fpath.exists()
    with open(fpath) as f:
        assert json.load(f) == {"arm": {"loss1.targ": "sp"}, "coord": {"opt.lr.init": 2.0e-5}}
    # the trial itself gets the merged set
    assert scheduled == [{"loss1.targ": "sp", "opt.lr.init": 2.0e-5}]
...
def test_run_campaign_defers_coord_dir_until_trial_launch(tmp_path, monkeypatch) -> None:
    ...
    cr.run_campaign("cmp_defer", "camp")
...
def test_run_campaign_marks_complete_after_successful_trial(tmp_path, monkeypatch) -> None:
    ...
    cr.run_campaign("cmp_complete", "camp")
...
def test_run_campaign_renders_tables_at_exit(tmp_path, monkeypatch, interrupted: bool) -> None:
    ...
    completed = cr.run_campaign("cmp_render", "camp")
...
def test_run_campaign_del_base_eval_cache_campaign_deletes_only_at_creation(tmp_path, monkeypatch) -> None:
    ...
    _set_camp(monkeypatch, n_trials_screen=1, n_trials_qual=None, trainval=False, datasets=("cub",), ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=_BASE_COORD)
    cr.run_campaign("cmp_delc", "camp")
...
def test_run_campaign_retries_then_fails_trial_without_progress(tmp_path, monkeypatch) -> None:
    ...
    with open(tmp_path / "cmp_fail" / "_screen" / "phase_metadata.json") as f:
...
def test_bump_crash_counts_reaches_coord_metadata(tmp_path) -> None:
    ...
    (dpath_phase / "phase_metadata.json").write_text(json.dumps({"n_crashes": {"ram": 0, "vram": 0, "other": 0}}))

    cr._bump_crash_counts(dpath_trial, dpath_phase, "vram")
    assert json.loads((dpath_phase / "phase_metadata.json").read_text())["n_crashes"] == {"ram": 0, "vram": 1, "other": 0}
    ...
    assert json.loads((dpath_phase / "phase_metadata.json").read_text())["n_crashes"] == {"ram": 1, "vram": 1, "other": 0}
...
def test_run_campaign_invalid_config_fails_at_kickoff(tmp_path, monkeypatch) -> None:
    ...
    with pytest.raises(ValueError, match="invalid config for arm 'bad' / coord 'base' on dataset 'cub'"):
        _set_camp(
            ...
            n_trials_screen=1, n_trials_qual=None, trainval=False,
            datasets=("cub",),
            ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}, {"loss1.targ": "mp", "name": "bad"}]],
            hpo_coords=_BASE_COORD,
        )
        cr.run_campaign("cmp_badcfg", "camp")
    assert scheduled == []
...
def test_run_campaign_retries_recover_across_flakes_that_make_progress(tmp_path, monkeypatch) -> None:
    ...
    cr.run_campaign("cmp_flaky", "camp")
    ...
    with open(tmp_path / "cmp_flaky" / "_screen" / "phase_metadata.json") as f:
...
def test_expand_combo_groups_raises_on_duplicate_names() -> None:
    with pytest.raises(ValueError, match="Duplicate hpo_coords name"):
        cr._expand_combo_groups(
            [
                [
                    {"loss1.targ": "sp", "name": "dup"},
                    {"loss1.targ": "phylo", "name": "dup"},
                ]
            ],
            "hpo_coords",
        )
...
def test_expand_combo_groups_single_combo_group_unchanged() -> None:
    ...
    members = cr._expand_combo_groups(
        [
            [
                {"loss.mix": 0.3, "loss2.targ": "phylo", "name": "hp"},
                {"loss1.targ": "sp", "name": "sp"},
            ]
        ],
        "ablation_arms",
    ...
    assert members == [
        ("hp", {"loss.mix": 0.3, "loss2.targ": "phylo"}),
        ("sp", {"loss1.targ": "sp"}),
    ]
...
def test_expand_combo_groups_derives_name_from_overrides_when_omitted() -> None:
    ...
    # (e.g. loss.mix -> Mix, mp -> MP) and anything unaliased passing through verbatim
    members = cr._expand_combo_groups(
        [
            [
                {"loss.mix": 0.3, "loss2.targ": "phylo"},
                {"loss1.targ": "mp"},
                {"loss1.targ": "sp", "name": "sp"},
                {"loss1.sim": "geo1"},
            ]
    ...
    assert members == [
        ("Mix-0.3_Targ2-hp", {"loss.mix": 0.3, "loss2.targ": "phylo"}),
        ("Targ-MP", {"loss1.targ": "mp"}),
        ("sp", {"loss1.targ": "sp"}),
        ("loss1.sim-geo1", {"loss1.sim": "geo1"}),
    ]
...
def test_expand_combo_groups_combo_list_expands_item_and_appends_to_name() -> None:
    ...
    members = cr._expand_combo_groups(
        [
            [
                {"loss.mix": 0.3, "loss2.targ": "phylo", "batch_size": [1024, 2048], "name": "hp"},
    ...
    assert members == [
        ("hp_BS-1k", {"loss.mix": 0.3, "loss2.targ": "phylo", "batch_size": 1024}),
        ("hp_BS-2k", {"loss.mix": 0.3, "loss2.targ": "phylo", "batch_size": 2048}),
...
def test_expand_combo_groups_multiple_combo_lists_cross_within_item() -> None:
    ...
    members = cr._expand_combo_groups(
        [
            [
                {"loss.mix": 0.3, "batch_size": [1024, 2048], "opt.lr.init": [7.0e-6, 2.0e-4], "name": "hp"},
            ]
        ],
        "hpo_coords",
    )
    ...
    assert dict(members)["hp_BS-2k_LR-2.0e-4"] == {"loss.mix": 0.3, "batch_size": 2048, "opt.lr.init": 2.0e-4}
...
def test_expand_combo_groups_combo_list_in_unnamed_item_folds_into_derived_name() -> None:
    ...
    members = cr._expand_combo_groups([[{"loss1.targ": "mp", "batch_size": [1024, 2048]}]], "ablation_arms")
    assert members == [
...
def test_expand_combo_groups_derived_and_explicit_names_join_across_combo_groups() -> None:
    ...
    assert dict(members) == {
        "hp_2k": {"loss.mix": 0.3, "loss2.targ": "phylo", "batch_size": 2048},
        "hp_BS-1k": {"loss.mix": 0.3, "loss2.targ": "phylo", "batch_size": 1024},
...
def test_expand_combo_groups_cartesian_product_merges_and_joins_names() -> None:
    ...
    assert dict(members) == {
        "hp_2k": {"loss.mix": 0.3, "loss2.targ": "phylo", "batch_size": 2048},
        "hp_1k": {"loss.mix": 0.3, "loss2.targ": "phylo", "batch_size": 1024},
    ...
    with pytest.raises(ValueError, match="ablation_arms key.*collide between combo groups"):
        cr._expand_combo_groups(
            [
                ...
                [
                    {"loss.mix": 0.4, "loss2.targ": "phylo", "name": "hp4"},
...
def test_expand_matrix_raises_on_arm_coord_key_collision() -> None:
    ...
    with pytest.raises(ValueError, match=r"batch_size.*appear in both ablation_arms and hpo_coords"):
        cr._expand_matrix(
            [[{"loss1.targ": "mp", "batch_size": 2048, "name": "mp"}]],
            [[{"batch_size": [1024, 2048]}]],
        )
    arms, coords = cr._expand_matrix(
        [[{"loss1.targ": "mp", "name": "mp"}, {"loss1.targ": "sp", "name": "sp"}]],
        [[{"batch_size": [1024, 2048]}]],
    )
    assert arms == [("mp", {"loss1.targ": "mp"}), ("sp", {"loss1.targ": "sp"})]
...
def test_expand_combo_groups_null_name_skips_component() -> None:
    ...
    members = cr._expand_combo_groups(
        [
            ...
            [
                {"loss1.wting.cls_imb.type": "inv_freq", "name": "if"},
                {"loss1.wting.cls_imb.type": "class_bal", "name": None},
            ],
        ],
        "ablation_arms",
    )
    assert dict(members) == {
        "hp_if": {"loss.mix": 0.3, "loss2.targ": "phylo", "loss1.wting.cls_imb.type": "inv_freq"},
        "hp": {"loss.mix": 0.3, "loss2.targ": "phylo", "loss1.wting.cls_imb.type": "class_bal"},
        "mp_if": {"loss1.targ": "mp", "loss1.wting.cls_imb.type": "inv_freq"},
        "mp": {"loss1.targ": "mp", "loss1.wting.cls_imb.type": "class_bal"},
    }
    ...
    with pytest.raises(ValueError, match="at most one item per combo group may set"):
        cr._expand_combo_groups(
            [
                [
                    {"loss1.targ": "mp", "name": "mp"},
                    {"loss1.targ": "sp", "name": None},
                    {"loss1.targ": "phylo", "name": None},
                ]
            ],
            "ablation_arms",
        )
...
def test_run_campaign_crosses_arms_with_coords(tmp_path, monkeypatch) -> None:
    ...
    def _fake_run_trial_subprocess(cfg_dict: dict, spare_render_pid=None):
        scheduled.append((cfg_dict["arm"], cfg_dict["coord"], cfg_dict["loss1"]["targ"], cfg_dict["loss1"]["sim"]))
        _leave_completed_trial(tmp_path, cfg_dict)
    ...
    _set_camp(
        ...
        n_trials_screen=1, n_trials_qual=None, trainval=False,
        datasets=("cub",),
        ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}, {"loss1.targ": "phylo", "name": "hp"}]],
        hpo_coords=[[{"loss1.sim": "cos", "name": "cos"}, {"loss1.sim": "l2", "name": "l2"}]],
    )
    cr.run_campaign("cmp_groups", "camp")
    ...
    with open(tmp_path / "cmp_groups" / "_screen" / "_datasets" / "cub" / "_arms" / "hp" / "_coords" / "l2" / "overrides.json") as f:
        data = json.load(f)
    assert data == {"arm": {"loss1.targ": "phylo"}, "coord": {"loss1.sim": "l2"}}
...
def test_manifest_completed_beats_stale_error_log(tmp_path) -> None:
    ...
    PrintLog.manifest(dpath_phase, [("cub", "hp", "c0", 42)], in_progress=None)
...
def test_manifest_shows_all_headers_at_kickoff(tmp_path) -> None:
    dpath_phase = Path(tmp_path) / "cmp_manifest_kickoff"
    dpath_phase.mkdir(parents=True)

    PrintLog.manifest(dpath_phase, [("cub", "hp", "c0", 42), ("lepid", "hp", "c0", 42)], in_progress=None)
...
def test_run_campaign_writes_manifest_tracking_outcomes(tmp_path, monkeypatch) -> None:
    ...
    def _fake_run_trial_subprocess(cfg_dict, spare_render_pid=None):
        ...
        in_progress_snapshots.append((cur, (dpath_phase / "manifest.log").read_text(encoding="utf-8")))
        d = _dpath_trial(tmp_path, cfg_dict)
        (d / "chkpts" / "in_progress").mkdir(parents=True, exist_ok=True)
        with open(d / "trial_metadata.json", "w") as f:
            json.dump({"dataset": cfg_dict["dataset"], "complete": False, "runtime": {"trial": "3661.0"}, "progress": {"epoch": 1, "n_epochs": 35, "n_samps_seen": 200_000}, "n_crashes": {"ram": 0, "vram": 0, "other": 0}}, f)
    ...
    cr.run_campaign("cmp_manifest_run", "camp")
...
def test_run_campaign_clears_in_progress_on_interrupt(tmp_path, monkeypatch) -> None:
    ...
    cr.run_campaign("cmp_manifest_interrupt", "camp")
...
def test_run_campaign_persists_and_grows_matrix(tmp_path, monkeypatch) -> None:
    ...
    with open(tmp_path / "cmp_grow" / "_screen" / "phase_metadata.json") as f:
        meta = json.load(f)
    assert meta["seeds"] == [42]
    assert meta["matrix"] == {"cub": {"sp": ["base"]}}  # the screening phase plans every coord under every arm
    ...
    _set_camp(
        ...
        hpo_coords=[[{"name": "base"}, {"loss1.sim": "geo1"}]],
    )
    cr.run_campaign("cmp_grow", "camp")
    ...
    assert meta["matrix"] == {d: {a: ["base", "loss1.sim-geo1"] for a in ("sp", "hp")} for d in ("cub", "lepid")}
...
def test_run_campaign_records_commit_hash_on_first_launch(tmp_path, monkeypatch) -> None:
    ...
    cr.run_campaign("cmp_commit", "camp")
...
def test_run_campaign_relaunch_survives_duration_only_metadata_rewrite(tmp_path, monkeypatch) -> None:
    ...
    cr.run_campaign("cmp_roundtrip", "camp")
    ...
    assert list(meta["matrix"]) == ["cub", "lepid"]


def test_run_campaign_prunes_removed_arm(tmp_path, monkeypatch) -> None:
    # a relaunch whose config drops an arm deletes the arm's dir (trials and stats) from the phase tree and records
    # the shrunk matrix; the kept arm's trials are untouched, nothing new runs, and the manifest drops the arm
    scheduled = _setup_completing_campaign(tmp_path, monkeypatch)
    _set_camp(monkeypatch, n_trials_screen=1, n_trials_qual=None, trainval=False, datasets=("cub",),
              ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}, {"loss1.targ": "phylo", "name": "hp"}]], hpo_coords=_BASE_COORD)
    cr.run_campaign("cmp_rm_arm", "camp")
    dpath_arms = tmp_path / "cmp_rm_arm" / "_screen" / "_datasets" / "cub" / "_arms"
    assert (dpath_arms / "hp" / "_coords" / "base" / "_seeds" / "42" / "trial_metadata.json").exists()

    del scheduled[:]
    _set_camp(monkeypatch, n_trials_screen=1, n_trials_qual=None, trainval=False, datasets=("cub",),
              ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=_BASE_COORD)
    cr.run_campaign("cmp_rm_arm", "camp")

    assert scheduled == []
    assert not (dpath_arms / "hp").exists()
    assert (dpath_arms / "sp" / "_coords" / "base" / "_seeds" / "42" / "trial_metadata.json").exists()
    meta = json.loads((tmp_path / "cmp_rm_arm" / "_screen" / "phase_metadata.json").read_text())
    assert meta["matrix"] == {"cub": {"sp": ["base"]}}
    text = (tmp_path / "cmp_rm_arm" / "_screen" / "manifest.log").read_text(encoding="utf-8")
    assert "cub/hp/" not in text and "cub/sp/base/42 ---" in text


def test_run_campaign_prunes_removed_coord(tmp_path, monkeypatch) -> None:
    # a coord removed from a combo list: its dir goes from every arm on every dataset, the sibling coord stays
    scheduled = _setup_completing_campaign(tmp_path, monkeypatch)
    _set_camp(monkeypatch, n_trials_screen=1, n_trials_qual=None, trainval=False, datasets=("cub",),
              ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=[[{"loss1.sim": ["cos", "geo1"]}]])
    cr.run_campaign("cmp_rm_coord", "camp")

    del scheduled[:]
    _set_camp(monkeypatch, n_trials_screen=1, n_trials_qual=None, trainval=False, datasets=("cub",),
              ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=[[{"loss1.sim": ["cos"]}]])
    cr.run_campaign("cmp_rm_coord", "camp")

    assert scheduled == []
    dpath_coords = tmp_path / "cmp_rm_coord" / "_screen" / "_datasets" / "cub" / "_arms" / "sp" / "_coords"
    assert not (dpath_coords / "loss1.sim-geo1").exists()
    assert (dpath_coords / "loss1.sim-cos" / "_seeds" / "42" / "trial_metadata.json").exists()
    meta = json.loads((tmp_path / "cmp_rm_coord" / "_screen" / "phase_metadata.json").read_text())
    assert meta["matrix"] == {"cub": {"sp": ["loss1.sim-cos"]}}


def test_run_campaign_prunes_removed_dataset(tmp_path, monkeypatch) -> None:
    scheduled = _setup_completing_campaign(tmp_path, monkeypatch)
    _set_camp(monkeypatch, n_trials_screen=1, n_trials_qual=None, trainval=False, datasets=("cub", "lepid"),
              ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=_BASE_COORD)
    cr.run_campaign("cmp_rm_dataset", "camp")
    ...
    assert scheduled == []
    dpath_datasets = tmp_path / "cmp_rm_dataset" / "_screen" / "_datasets"
    assert not (dpath_datasets / "lepid").exists()
    assert (dpath_datasets / "cub" / "_arms" / "sp" / "_coords" / "base" / "_seeds" / "42" / "trial_metadata.json").exists()
    meta = json.loads((tmp_path / "cmp_rm_dataset" / "_screen" / "phase_metadata.json").read_text())
    assert meta["matrix"] == {"cub": {"sp": ["base"]}}
...
def test_run_campaign_raises_on_removed_seed(tmp_path, monkeypatch) -> None:
    ...
    scheduled = _setup_completing_campaign(tmp_path, monkeypatch)
    _set_camp(monkeypatch, n_trials_screen=2, n_trials_qual=None, trainval=False, datasets=("cub",),
              ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=_BASE_COORD)
...
def test_run_campaign_use_img_cache_missing_pack_errors_before_trials(tmp_path, monkeypatch) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="cub"):
        ...
        cr.run_campaign("cmp_ic_missing", "camp")
    assert launched == []
...
def test_run_campaign_use_img_cache_records_staging_runtime(tmp_path, monkeypatch) -> None:
    ...
    cr.run_campaign("cmp_ic_rt", "camp")

    meta = json.loads((tmp_path / "cmp_ic_rt" / "_screen" / "phase_metadata.json").read_text())
...
def test_run_campaign_use_img_cache_override_checked_at_startup(tmp_path, monkeypatch, side: str) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="cub"):
        _set_camp(
            ...
            n_trials_screen=1, n_trials_qual=None, trainval=False,
            datasets=("cub",),
            ablation_arms=ic if side == "arm" else [[{"loss1.targ": "sp", "name": "sp"}]],
            hpo_coords=ic if side == "coord" else _BASE_COORD,
        )
        cr.run_campaign("cmp_ic_override", "camp")
    assert launched == []


def _resolve_campaign(tmp_path, monkeypatch, continue_campaign, suffix=None) -> str:
    """cr._resolve_campaign('dev') with stubbed config loading; returns the campaign name it resolves to."""
    ...
    return cr._resolve_campaign("dev")


def test_resolve_campaign_continue_campaign_true_keeps_existing_name(tmp_path, monkeypatch) -> None:
    (tmp_path / "dev").mkdir()
    assert _resolve_campaign(tmp_path, monkeypatch, continue_campaign=True) == "dev"


def test_resolve_campaign_continue_campaign_false_keeps_name_without_collision(tmp_path, monkeypatch) -> None:
    assert _resolve_campaign(tmp_path, monkeypatch, continue_campaign=False) == "dev"


def test_resolve_campaign_continue_campaign_false_dedupes_to_first_free_name(tmp_path, monkeypatch) -> None:
    (tmp_path / "dev").mkdir()
    (tmp_path / "dev2").mkdir()
    assert _resolve_campaign(tmp_path, monkeypatch, continue_campaign=False) == "dev3"


def test_resolve_campaign_continue_campaign_false_dedupes_suffixed_name(tmp_path, monkeypatch) -> None:
    (tmp_path / "dev_foobar").mkdir()
    assert _resolve_campaign(tmp_path, monkeypatch, continue_campaign=False, suffix="foobar") == "dev_foobar2"


def _wire_queue(tmp_path, monkeypatch) -> Path:
    """Point the runner at a real config dir (camp_queue.yaml + camps/ live under it) and an empty artifacts dir,
    resume campaigns by name (dev.continue_campaign) and neutralize pytest's own argv; returns the config dir."""
    dpath_config = tmp_path / "config"
    (dpath_config / "camps").mkdir(parents=True)
    monkeypatch.setattr(cr, "paths", {"config": dpath_config, "artifacts": tmp_path / "artifacts"})
    monkeypatch.setattr(cr, "load_train_config_dict", lambda: {"dev": {"continue_campaign": True}})
...
def _add_camp_config(dpath_config: Path, *names: str, text: str = _VALID_CAMP_YAML) -> None:
    for name in names:
        (dpath_config / "camps" / f"{name}.yaml").write_text(text)


def _stub_runs(monkeypatch, on_run=None, plan_changed=lambda run: False) -> list[str]:
    """Stub cr.run_campaign to just record the campaigns launched (returning True) and cr._plan_changed with
    `plan_changed(run)` (nothing changed, by default); `on_run(campaign)` can rewrite camp_queue.yaml mid-run to
    simulate live edits. Returns the record list."""
    ran = []

    def _fake_run_campaign(campaign: str, name: str) -> bool:
        ran.append(campaign)
        if on_run is not None:
            on_run(campaign)
        return True

    monkeypatch.setattr(cr, "run_campaign", _fake_run_campaign)
    monkeypatch.setattr(cr, "_plan_changed", plan_changed)
    return ran
...
def test_main_runs_queue_in_order_and_picks_up_added_entries(tmp_path, monkeypatch) -> None:
    ...
    def _append_during_a(campaign: str) -> None:
        if campaign == "a":
            _write_queue(dpath_config, ["camp.a", "camp.b", "camp.c"])

    ran = _stub_runs(monkeypatch, on_run=_append_during_a)
    cr.main()
    assert ran == ["a", "b", "c"]
...
def test_main_picks_up_entries_inserted_at_any_position(tmp_path, monkeypatch) -> None:
    ...
    def _insert_during_a(campaign: str) -> None:
        if campaign == "a":
            _write_queue(dpath_config, ["camp.c", "camp.a", "camp.d", "camp.b"])

    ran = _stub_runs(monkeypatch, on_run=_insert_during_a)
    cr.main()
    assert ran == ["a", "c", "d", "b"]
...
def test_main_relaunches_a_run_campaign_whose_plan_changed_before_the_next_entry(tmp_path, monkeypatch) -> None:
    dpath_config = _wire_queue(tmp_path, monkeypatch)
    _add_camp_config(dpath_config, "a", "b", "c")
    _write_queue(dpath_config, ["camp.a", "camp.b", "camp.c"])
    # a's dir already exists and dev.continue_campaign is off, so a runs as 'a2': the relaunch must resume that same
    # dir rather than resolve the name afresh (which, a2's dir existing by then, would start 'a3')
    (tmp_path / "artifacts" / "a").mkdir(parents=True)
    monkeypatch.setattr(cr, "load_train_config_dict", lambda: {"dev": {"continue_campaign": False}})

    # a's yaml changes its plan while b runs: a is relaunched once b is over, before c is launched
    edited = set()

    def _on_run(campaign: str) -> None:
        (tmp_path / "artifacts" / campaign).mkdir(exist_ok=True)  # the run creates its dir, as run_campaign would
        if campaign == "b":
            edited.add("a2")

    def _plan_changed(run) -> bool:
        if run.campaign not in edited:
            return False
        edited.remove(run.campaign)
        return True

    ran = _stub_runs(monkeypatch, on_run=_on_run, plan_changed=_plan_changed)
    cr.main()
    assert ran == ["a2", "b", "a2", "c"]


_PLAN_SPEC = dict(
    n_trials_screen=1, n_trials_qual=None, trainval=False, datasets=("cub",),
    ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}, {"loss1.targ": "phylo", "name": "hp"}]], hpo_coords=_BASE_COORD,
)
...
@pytest.mark.parametrize("edit, expected", [
    ({}, False),
    ({"hpo_coords": [[{"name": "base"}, {"loss1.sim": "geo1"}]]}, True),
    ({"n_trials_screen": 2}, True),
    ({"datasets": ("cub", "lepid")}, True),
    ({"ablation_arms": [[{"loss1.targ": "sp", "name": "sp"}]]}, True),
    ({"n_trials_qual": 1}, True),
], ids=["unchanged", "coord_added", "seed_added", "dataset_added", "arm_removed", "qual_switched_on"])
def test_plan_changed_compares_the_yaml_to_the_recorded_plans(tmp_path, monkeypatch, edit, expected) -> None:
    ...
    # the qual plan picks each arm's screening best (report.pick_best_coords reads real stats)
    monkeypatch.setattr(cr, "pick_best_coords", lambda: {("sp", "cub"): "base", ("hp", "cub"): "base"})

    _set_camp(monkeypatch, **{**_PLAN_SPEC, **edit})
...
def test_plan_changed_tolerates_an_invalid_yaml(tmp_path, monkeypatch, capsys) -> None:
    ...
    assert "camp_queue: 'camp.camp' config invalid -- not relaunched until it is fixed" in capsys.readouterr().out


def test_plan_changed_stops_at_an_incomplete_earlier_phase(tmp_path, monkeypatch) -> None:
    """A campaign stuck at a screening trial that failed for good is relaunched for a screening addition, not for a
    qual phase it can't reach yet -- else the queue would relaunch it over and over."""
    _setup_completing_campaign(tmp_path, monkeypatch)
    run_trial = cr._run_trial_subprocess

    def _fail_hp(cfg_dict, spare_render_pid=None):
        if cfg_dict["arm"] != "hp":
            return run_trial(cfg_dict, spare_render_pid)
        d = _dpath_trial(tmp_path, cfg_dict)
        d.mkdir(parents=True, exist_ok=True)
        with open(d / "trial_metadata.json", "w") as f:
            json.dump({"dataset": "cub", "complete": False, "runtime": {"trial": "3661.0"}, "progress": {"epoch": 1, "n_epochs": 35, "n_samps_seen": 200_000}, "n_crashes": {"ram": 0, "vram": 0, "other": 0}}, f)
    ...
    _set_camp(monkeypatch, **_PLAN_SPEC)
    cr.run_campaign("cmp_plan_fail", "camp")
    run = cr._Run("camp.camp", "cmp_plan_fail", "camp")
    ...
    _set_camp(monkeypatch, **{**_PLAN_SPEC, "n_trials_qual": 1})
    assert cr._plan_changed(run) is False

    _set_camp(monkeypatch, **{**_PLAN_SPEC, "hpo_coords": [[{"name": "base"}, {"loss1.sim": "geo1"}]]})
    assert cr._plan_changed(run) is True


def test_main_runs_the_trials_added_to_a_finished_campaign_after_the_running_one(tmp_path, monkeypatch) -> None:
    """End to end: a runs, then b; while b runs, a's yaml gains a coord. Once b is over, a is relaunched for the added
    trial -- its recorded matrix grows -- and the queue drains."""
    dpath_config = _wire_queue(tmp_path, monkeypatch)
    _write_queue(dpath_config, ["camp.a", "camp.b"])
    scheduled = _setup_completing_campaign(tmp_path, monkeypatch)  # artifacts under tmp_path itself
    monkeypatch.setattr(cr, "paths", {**cr.paths, "config": dpath_config})

    def _camp(name: str, coords: list) -> CampaignConfig:
        return CampaignConfig(suffix=None, n_trials_screen=1, n_trials_qual=None, trainval=False, datasets=("cub",),
                              ablation_arms=[[{"loss1.targ": "sp", "name": f"sp_{name}"}]], hpo_coords=coords)

    cfgs = {name: _camp(name, _BASE_COORD) for name in ("a", "b")}
    monkeypatch.setattr(cr, "_load_campaign_config", lambda name: cfgs[name])
    run_trial = cr._run_trial_subprocess

    def _edit_a_during_b(cfg_dict, spare_render_pid=None):
        run_trial(cfg_dict, spare_render_pid)
        if cfg_dict["campaign"] == "b":
            cfgs["a"] = _camp("a", [[{"name": "base"}, {"loss1.sim": "geo1"}]])

    monkeypatch.setattr(cr, "_run_trial_subprocess", _edit_a_during_b)
    cr.main()
    assert scheduled == [("sp_a", "base", "cub", 42), ("sp_b", "base", "cub", 42), ("sp_a", "loss1.sim-geo1", "cub", 42)]
    with open(tmp_path / "a" / "_screen" / "phase_metadata.json") as f:
        assert json.load(f)["matrix"] == {"cub": {"sp_a": ["base", "loss1.sim-geo1"]}}
...
def test_load_campaign_config_rejects_screen_exceeding_qual(tmp_path, monkeypatch, capsys) -> None:
    # the config layer (CampaignConfig) refuses n_trials_screen > n_trials_qual: a hard failure for the entry about
    # to run, only a warning while it sits further down the queue
    dpath_config = _wire_queue(tmp_path, monkeypatch)
    _add_camp_config(dpath_config, "a")
    _add_camp_config(dpath_config, "bad", text=_VALID_CAMP_YAML.replace("n_trials_screen: 1", "n_trials_screen: 3").replace("n_trials_qual: null", "n_trials_qual: 2"))
    with pytest.raises(ValueError, match="n_trials_screen"):
        cr._load_campaign_config("bad")
    assert cr._load_campaign_config("a").n_trials_qual is None

    _write_queue(dpath_config, ["camp.a", "camp.bad"])
    ran = _stub_runs(monkeypatch)
    with pytest.raises(ValueError, match="n_trials_screen"):
        cr.main()
    assert ran == ["a"]
    assert "pending entry 'camp.bad' is invalid" in capsys.readouterr().out


def _setup_phased_campaign(tmp_path, monkeypatch, picks: dict) -> list:
    """Wire run_campaign for a screening + qual run without real subprocesses/renders: fake trials complete
    cleanly (as _setup_completing_campaign), and the qual selection (report.pick_best_coords, which reads real
    eval files) is stubbed to return `picks` ({(arm, dataset): coord}). Returns the list of
    (phase, dataset, arm, coord, seed) tuples each launched trial was invoked with."""
    monkeypatch.setattr(cr, "SEED0", 42)
    monkeypatch.setattr(cr, "paths", {"artifacts": tmp_path, "imgs": {}, "img_cache": tmp_path / "img_cache"})
    _stub_campaign_config(monkeypatch)
    monkeypatch.setattr(cr, "pick_best_coords", lambda: dict(picks))

    scheduled: list = []

    def _fake_run_trial_subprocess(cfg_dict: dict, spare_render_pid=None):
        scheduled.append((cfg_dict["phase"], cfg_dict["dataset"], cfg_dict["arm"], cfg_dict["coord"], cfg_dict["seed"]))
        _leave_completed_trial(tmp_path, cfg_dict)
...
_ARMS_SP_HP = [[{"loss1.targ": "sp", "name": "sp"}, {"loss1.targ": "phylo", "name": "hp"}]]
_COORDS_SIM = [[{"loss1.sim": ["cos", "geo1"]}]]  # -> loss1.sim-cos, loss1.sim-geo1


def test_run_campaign_qual_copies_picks_and_tops_up_seeds(tmp_path, monkeypatch) -> None:
    # after the screening matrix (2 arms x 2 coords x 1 seed) the qual phase takes each arm's pick, copies its
    # screening coord dir wholesale into qual/ (the seed-42 trial arrives complete, with overrides.json) and runs
    # the remaining seeds 43, 44 there -- seed-major, arms in campaign order -- so the qual tree reads as if
    # n_trials_qual = 3 trials had run for each pick; the unpicked coords never appear in qual/, and the
    # screening tree is left untouched. The qual phase records its own matrix/coords/seeds and a copy of the
    # frozen config snapshot.
    scheduled = _setup_phased_campaign(tmp_path, monkeypatch, {("sp", "cub"): "loss1.sim-geo1", ("hp", "cub"): "loss1.sim-cos"})

    _set_camp(
        monkeypatch,
        n_trials_screen=1, n_trials_qual=3, trainval=False, datasets=("cub",),
                           ablation_arms=_ARMS_SP_HP, hpo_coords=_COORDS_SIM
    )
    assert cr.run_campaign("cmp_qual", "camp")

    assert scheduled[:4] == [("_screen", "cub", "sp", "loss1.sim-cos", 42), ("_screen", "cub", "sp", "loss1.sim-geo1", 42),
                             ("_screen", "cub", "hp", "loss1.sim-cos", 42), ("_screen", "cub", "hp", "loss1.sim-geo1", 42)]
    assert scheduled[4:] == [("qual", "cub", "sp", "loss1.sim-geo1", 43), ("qual", "cub", "hp", "loss1.sim-cos", 43),
                             ("qual", "cub", "sp", "loss1.sim-geo1", 44), ("qual", "cub", "hp", "loss1.sim-cos", 44)]

    dpath_qual = tmp_path / "cmp_qual" / "qual"
    dpath_pick = dpath_qual / "_datasets" / "cub" / "_arms" / "sp" / "_coords" / "loss1.sim-geo1"
    ...
    assert json.loads((dpath_pick / "overrides.json").read_text()) == {"arm": {"loss1.targ": "sp"}, "coord": {"loss1.sim": "geo1"}}
    assert not (dpath_qual / "_datasets" / "cub" / "_arms" / "sp" / "_coords" / "loss1.sim-cos").exists()
    assert (dpath_qual / "_datasets" / "cub" / "_arms" / "hp" / "_coords" / "loss1.sim-cos" / "_seeds" / "44").exists()
    assert not (dpath_qual / "_datasets" / "cub" / "_arms" / "hp" / "_coords" / "loss1.sim-geo1").exists()
    assert (dpath_qual / "cfg_baseline.json").exists()
    meta = json.loads((dpath_qual / "phase_metadata.json").read_text())
    assert meta["matrix"] == {"cub": {"sp": ["loss1.sim-geo1"], "hp": ["loss1.sim-cos"]}}
    ...
    assert "cub/sp/loss1.sim-geo1/42 ---" in text and "cub/sp/loss1.sim-cos" not in text
    ...
    assert (dpath_screen / "loss1.sim-cos" / "_seeds" / "42").exists() and (dpath_screen / "loss1.sim-geo1" / "_seeds" / "42").exists()
    assert not (dpath_screen / "loss1.sim-geo1" / "_seeds" / "43").exists()
    assert json.loads((tmp_path / "cmp_qual" / "_screen" / "phase_metadata.json").read_text())["seeds"] == [42]


def test_run_campaign_qual_null_skips_qual(tmp_path, monkeypatch) -> None:
    scheduled = _setup_phased_campaign(tmp_path, monkeypatch, {("sp", "cub"): "loss1.sim-geo1"})
    monkeypatch.setattr(cr, "pick_best_coords", lambda: pytest.fail("no pick with n_trials_qual null"))

    _set_camp(
        monkeypatch,
        n_trials_screen=1, n_trials_qual=None, trainval=False, datasets=("cub",),
                           ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=_COORDS_SIM
    )
    assert cr.run_campaign("cmp_noqual", "camp")

    assert [t[0] for t in scheduled] == ["_screen", "_screen"]
    assert not (tmp_path / "cmp_noqual" / "qual").exists()


def test_run_campaign_qual_equal_counts_only_copies(tmp_path, monkeypatch) -> None:
    # n_trials_qual == n_trials_screen: the qual phase is the copy of each pick plus its stats, no new trials
    scheduled = _setup_phased_campaign(tmp_path, monkeypatch, {("sp", "cub"): "loss1.sim-cos"})

    _set_camp(
        monkeypatch,
        n_trials_screen=1, n_trials_qual=1, trainval=False, datasets=("cub",),
                           ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=_COORDS_SIM
    )
    assert cr.run_campaign("cmp_eq", "camp")

    assert [t[0] for t in scheduled] == ["_screen", "_screen"]
    dpath_qual = tmp_path / "cmp_eq" / "qual"
    assert (dpath_qual / "_datasets" / "cub" / "_arms" / "sp" / "_coords" / "loss1.sim-cos" / "_seeds" / "42" / "trial_metadata.json").exists()
...
def test_run_campaign_qual_adds_new_best_pick_on_relaunch(tmp_path, monkeypatch) -> None:
    ...
    _set_camp(
        monkeypatch,
        n_trials_screen=1, n_trials_qual=2, trainval=False, datasets=("cub",),
                           ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=_COORDS_SIM
    )
    assert cr.run_campaign("cmp_addpick", "camp")
    assert scheduled[2:] == [("qual", "cub", "sp", "loss1.sim-geo1", 43)]
    dpath_qual = tmp_path / "cmp_addpick" / "qual"
    (dpath_qual / "_datasets" / "cub" / "_arms" / "sp" / "_coords" / "loss1.sim-geo1" / "marker").write_text("kept")  # survives: no re-copy

    del scheduled[:]
    monkeypatch.setattr(cr, "pick_best_coords", lambda: {("sp", "cub"): "loss1.sim-cos", ("hp", "cub"): "loss1.sim-cos"})
    _set_camp(
        monkeypatch,
        n_trials_screen=1, n_trials_qual=3, trainval=False, datasets=("cub",),
                           ablation_arms=_ARMS_SP_HP, hpo_coords=_COORDS_SIM
    ...
    assert scheduled == [
        ("_screen", "cub", "hp", "loss1.sim-cos", 42), ("_screen", "cub", "hp", "loss1.sim-geo1", 42),
        ("qual", "cub", "sp", "loss1.sim-cos", 43), ("qual", "cub", "hp", "loss1.sim-cos", 43),
        ("qual", "cub", "sp", "loss1.sim-geo1", 44), ("qual", "cub", "sp", "loss1.sim-cos", 44), ("qual", "cub", "hp", "loss1.sim-cos", 44),
    ]
    meta = json.loads((dpath_qual / "phase_metadata.json").read_text())
    assert meta["matrix"] == {"cub": {"sp": ["loss1.sim-geo1", "loss1.sim-cos"], "hp": ["loss1.sim-cos"]}}
    assert meta["seeds"] == [42, 43, 44]
    assert (dpath_qual / "_datasets" / "cub" / "_arms" / "sp" / "_coords" / "loss1.sim-geo1" / "marker").read_text() == "kept"
    # sp's added pick came over from screening with its completed seed-42 trial
    dpath_cos = dpath_qual / "_datasets" / "cub" / "_arms" / "sp" / "_coords" / "loss1.sim-cos"
...
def test_run_campaign_stops_after_incomplete_phase(tmp_path, monkeypatch, capsys) -> None:
    ...
    scheduled = _setup_phased_campaign(tmp_path, monkeypatch, {("sp", "cub"): "loss1.sim-cos", ("hp", "cub"): "loss1.sim-cos"})
    monkeypatch.setattr(cr, "pick_best_coords", lambda: pytest.fail("no pick from an incomplete screening phase"))
    fake_ok = cr._run_trial_subprocess

    def _fail_hp_geo1(cfg_dict, spare_render_pid=None):
        if (cfg_dict["arm"], cfg_dict["coord"]) == ("hp", "loss1.sim-geo1"):
            raise subprocess.CalledProcessError(1, ["torchrun"], stderr="boom")
    ...
    assert cr.run_campaign("cmp_halt", "camp") is True
    ...
    assert "cub/hp/loss1.sim-geo1/42 ---" in (tmp_path / "cmp_halt" / "_screen" / "manifest.log").read_text(encoding="utf-8").split("Completed")[0]


def test_run_campaign_removed_coord_drops_its_qual_pick(tmp_path, monkeypatch) -> None:
    # a relaunch that removes a picked coord drops the pick: its qual dir (copied trial + qual trials) and its
    ...
    scheduled = _setup_phased_campaign(tmp_path, monkeypatch, {("sp", "cub"): "loss1.sim-geo1"})
    _set_camp(monkeypatch, n_trials_screen=1, n_trials_qual=2, trainval=False, datasets=("cub",),
              ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=_COORDS_SIM)
    assert cr.run_campaign("cmp_rm_pick", "camp")
    assert scheduled[2:] == [("qual", "cub", "sp", "loss1.sim-geo1", 43)]

    del scheduled[:]
    monkeypatch.setattr(cr, "pick_best_coords", lambda: {("sp", "cub"): "loss1.sim-cos"})
    _set_camp(monkeypatch, n_trials_screen=1, n_trials_qual=2, trainval=False, datasets=("cub",),
              ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=[[{"loss1.sim": ["cos"]}]])
    assert cr.run_campaign("cmp_rm_pick", "camp")

    assert scheduled == [("qual", "cub", "sp", "loss1.sim-cos", 43)]
    dpath_coords_qual = tmp_path / "cmp_rm_pick" / "qual" / "_datasets" / "cub" / "_arms" / "sp" / "_coords"
    assert not (dpath_coords_qual / "loss1.sim-geo1").exists()
    assert not (tmp_path / "cmp_rm_pick" / "_screen" / "_datasets" / "cub" / "_arms" / "sp" / "_coords" / "loss1.sim-geo1").exists()
    assert json.loads((dpath_coords_qual / "loss1.sim-cos" / "_seeds" / "42" / "trial_metadata.json").read_text())["complete"] is True
    meta = json.loads((tmp_path / "cmp_rm_pick" / "qual" / "phase_metadata.json").read_text())
    assert meta["matrix"] == {"cub": {"sp": ["loss1.sim-cos"]}}


def test_run_campaign_live_edit_adds_trials_between_trials(tmp_path, monkeypatch) -> None:
    # the camp yaml is re-read before every trial: an arm added while the first trial runs is planned from the next
    # trial on -- its trials slot into launch order (seed-major, so its seed-42 trial runs before the seed-43 ones)
    # and the matrix records it
    scheduled = _setup_completing_campaign(tmp_path, monkeypatch)
    _set_camp(monkeypatch, n_trials_screen=2, n_trials_qual=None, trainval=False, datasets=("cub",),
              ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=_BASE_COORD)
    fake_ok = cr._run_trial_subprocess

    def _add_hp_during_first(cfg_dict, spare_render_pid=None):
        fake_ok(cfg_dict, spare_render_pid)
        if len(scheduled) == 1:  # saved while the first trial runs
            _set_camp(monkeypatch, n_trials_screen=2, n_trials_qual=None, trainval=False, datasets=("cub",),
                      ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}, {"loss1.targ": "phylo", "name": "hp"}]], hpo_coords=_BASE_COORD)

    monkeypatch.setattr(cr, "_run_trial_subprocess", _add_hp_during_first)
    assert cr.run_campaign("cmp_live_add", "camp")

    assert scheduled == [("sp", "base", "cub", 42), ("hp", "base", "cub", 42), ("sp", "base", "cub", 43), ("hp", "base", "cub", 43)]
    meta = json.loads((tmp_path / "cmp_live_add" / "_screen" / "phase_metadata.json").read_text())
    assert meta["matrix"] == {"cub": {"sp": ["base"], "hp": ["base"]}}
    text = (tmp_path / "cmp_live_add" / "_screen" / "manifest.log").read_text(encoding="utf-8")
    assert "cub/hp/base/43 ---" in text


def test_run_campaign_live_edit_removes_items_between_trials(tmp_path, monkeypatch) -> None:
    # a coord removed while a trial runs is pruned from the next trial on: its completed trial's dir goes, its queued
    ...
    assert scheduled == [("sp", "loss1.sim-cos", "cub", 42), ("sp", "loss1.sim-geo1", "cub", 42), ("sp", "loss1.sim-cos", "cub", 43)]
    dpath_coords = tmp_path / "cmp_live_rm" / "_screen" / "_datasets" / "cub" / "_arms" / "sp" / "_coords"
    assert not (dpath_coords / "loss1.sim-geo1").exists()
    assert (dpath_coords / "loss1.sim-cos" / "_seeds" / "43" / "trial_metadata.json").exists()
    meta = json.loads((tmp_path / "cmp_live_rm" / "_screen" / "phase_metadata.json").read_text())
    assert meta["matrix"] == {"cub": {"sp": ["loss1.sim-cos"]}}
    assert "geo1" not in (tmp_path / "cmp_live_rm" / "_screen" / "manifest.log").read_text(encoding="utf-8")


def test_run_campaign_live_edit_during_qual_screens_added_arm_first(tmp_path, monkeypatch) -> None:
    # an arm added while the qual phase runs needs screening before it can be picked: the qual phase hands back, the
    # screening phase runs the new arm's trials (the old ones are complete), then qual resumes with the new arm's
    ...
    def _add_hp_during_first_qual_trial(cfg_dict, spare_render_pid=None):
        fake_ok(cfg_dict, spare_render_pid)
        if cfg_dict["phase"] == "qual" and len(scheduled) == 3:
            _set_camp(monkeypatch, n_trials_screen=1, n_trials_qual=2, trainval=False, datasets=("cub",),
                      ablation_arms=_ARMS_SP_HP, hpo_coords=_COORDS_SIM)

    monkeypatch.setattr(cr, "_run_trial_subprocess", _add_hp_during_first_qual_trial)
    assert cr.run_campaign("cmp_live_qual", "camp")

    assert scheduled == [
        ("_screen", "cub", "sp", "loss1.sim-cos", 42), ("_screen", "cub", "sp", "loss1.sim-geo1", 42),
        ("qual", "cub", "sp", "loss1.sim-geo1", 43),
        ("_screen", "cub", "hp", "loss1.sim-cos", 42), ("_screen", "cub", "hp", "loss1.sim-geo1", 42),
        ("qual", "cub", "hp", "loss1.sim-cos", 43),
    ]
    meta = json.loads((tmp_path / "cmp_live_qual" / "qual" / "phase_metadata.json").read_text())
    assert meta["matrix"] == {"cub": {"sp": ["loss1.sim-geo1"], "hp": ["loss1.sim-cos"]}}
    assert json.loads((tmp_path / "cmp_live_qual" / "_screen" / "phase_metadata.json").read_text())["matrix"] == {
        "cub": {"sp": ["loss1.sim-cos", "loss1.sim-geo1"], "hp": ["loss1.sim-cos", "loss1.sim-geo1"]}
    }


def test_run_campaign_live_edit_switches_qual_off(tmp_path, monkeypatch) -> None:
    # n_trials_qual set to null while the qual phase runs ends the campaign after the running trial; the qual tree
    # is left as is
    scheduled = _setup_phased_campaign(tmp_path, monkeypatch, {("sp", "cub"): "loss1.sim-geo1"})
    _set_camp(monkeypatch, n_trials_screen=1, n_trials_qual=3, trainval=False, datasets=("cub",),
              ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=_COORDS_SIM)
    fake_ok = cr._run_trial_subprocess

    def _switch_qual_off(cfg_dict, spare_render_pid=None):
        fake_ok(cfg_dict, spare_render_pid)
        if cfg_dict["phase"] == "qual":
            _set_camp(monkeypatch, n_trials_screen=1, n_trials_qual=None, trainval=False, datasets=("cub",),
                      ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=_COORDS_SIM)

    monkeypatch.setattr(cr, "_run_trial_subprocess", _switch_qual_off)
    assert cr.run_campaign("cmp_live_off", "camp")

    assert [t for t in scheduled if t[0] == "qual"] == [("qual", "cub", "sp", "loss1.sim-geo1", 43)]
    assert (tmp_path / "cmp_live_off" / "qual" / "_datasets" / "cub" / "_arms" / "sp" / "_coords" / "loss1.sim-geo1" / "_seeds" / "43" / "trial_metadata.json").exists()
...
def test_run_campaign_live_edit_invalid_yaml_keeps_last_plan(tmp_path, monkeypatch, capsys) -> None:
    ...
    scheduled = _setup_completing_campaign(tmp_path, monkeypatch)
    _set_camp(monkeypatch, n_trials_screen=3, n_trials_qual=None, trainval=False, datasets=("cub",),
              ablation_arms=[[{"loss1.targ": "sp", "name": "sp"}]], hpo_coords=_BASE_COORD)
    fake_ok = cr._run_trial_subprocess
    arms_sp_hp = [[{"loss1.targ": "sp", "name": "sp"}, {"loss1.targ": "phylo", "name": "hp"}]]

    def _unparseable(name):
        raise yaml.YAMLError("mapping values are not allowed here")

    def _edit_between_trials(cfg_dict, spare_render_pid=None):
        fake_ok(cfg_dict, spare_render_pid)
        if len(scheduled) == 1:
    ...
    monkeypatch.setattr(cr, "_run_trial_subprocess", _edit_between_trials)
    assert cr.run_campaign("cmp_live_bad", "camp")

    assert scheduled == [("sp", "base", "cub", 42), ("sp", "base", "cub", 43), ("sp", "base", "cub", 44),
    ...
    meta = json.loads((tmp_path / "cmp_live_bad" / "_screen" / "phase_metadata.json").read_text())
