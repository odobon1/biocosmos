...
1. Define the campaign in `config/camps/<campaign>.yaml`:
   ...
   **Phases.** A campaign runs in two phases, each with its own artifact tree under `artifacts/<campaign>/<phase>/` (below). The **screening** phase is the full matrix above. Once it has run, the **qual** phase — skipped when `n_trials_qual` is `null` — picks, for each arm on each dataset, its **best coord** from the screening results: the coord with the highest across-trial mean Native mAP composite All score (`comp.map.all` at the mAP-selected checkpoint; ties to the first coord in campaign order; an arm with no completed screening trial on a dataset sits the qual phase out there). The pick's whole coord dir is copied from `screening/` into `qual/` — its screening trials, `config.json` / `overrides.json` / `coord_metadata.json` and `coord_stats/` — and the qual phase then runs seeds `SEED0 + n_trials_screen .. SEED0 + n_trials_qual - 1` for it, so the qual tree reads as if `n_trials_qual` trials had run for each pick from the start (its `coord_stats/` and every table re-aggregate over all of them as the new trials complete; with `n_trials_qual == n_trials_screen` it is just the copy plus its stats). Picks are frozen in `qual/campaign_metadata.json` at the qual phase's first launch: a relaunch reuses them even if the screening results have since moved, and only an arm or dataset added later is picked fresh. Each phase's `campaign_metadata.json` records its planned **matrix** (`{dataset: {arm: [coords]}}` — every coord in screening, the one pick in qual), which is what the stats code keys its rows and render cycles off.
   ...
   Queued as `camp.foobar` and launched via `python -m campaign_runner`, this runs **60 trials**: 3 arms (`sp`, `mp`, `hp`) × 2 coords (`LR-1.0e-5`, `LR-2.0e-5`) × 2 datasets (`cub`, `lepid`) × 5 seeds (`42`–`46` — trial seeds are `SEED0 .. SEED0 + n_trials_screen - 1`, and `SEED0 = 42`), seed-major (then dataset, arm, coord). Each trial's artifacts land under `artifacts/foobar/screening/datasets/<dataset>/arms/<arm>/coords/<coord>/<seed>/` (the qual phase's under `artifacts/foobar/qual/…`); each phase's tree:
    ```
    artifacts/<campaign>/<phase>/               screening/ and, with n_trials_qual set, qual/ -- the same tree each
      datasets/<dataset>/
        arms/<arm>/
          coords/<coord>/
            <seed>/                          one trial: evals/, learning_curves/, logs/, data_trial.pkl, trial_metadata.json
...
| 3 | Campaign runner injections | `run_campaign()` | Injects `campaign`, `phase`, `arm`, `coord`, `seed`, `dataset` from the campaign matrix (these per-trial keys exist only here, not in `train.yaml`). |
