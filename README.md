...
`manifold_viz.yaml`'s `tsne.perplexity` is a list (e.g. `[15, 30, 60]`), and a t-SNE is generated at every value in it. With a single value the t-SNE dirs are exactly as above; with more than one, each perplexity gets its own t-SNE method dirs and 4panel dir, suffixed by the perplexity — e.g. `2panel/tsne_15/`, `7panel/tsne_30/`, and `4panel_60/` — while the perplexity-independent PCA dirs (`{2,7}panel/pca/`) are unchanged. This applies to both the per-eval plots and the evolution GIFs. (Changing the perplexity list invalidates cached `projections.npz`; regenerate the affected evals — including the shared `base_eval_cache`.)
