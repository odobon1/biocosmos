...
from utils.config import load_aliases_config_dict
from utils.config import CampaignConfig
from utils.config import group_path
from utils.report import fpath_workbook
...
def _set_camp(monkeypatch, **spec) -> None:
    ...
    file would."""
    cfg = CampaignConfig(suffix=None, baseline_overrides={}, **spec)
...
def _dpath_trial(tmp_path, cfg_dict) -> Path:
    return (tmp_path / cfg_dict["campaign"] / "_phase" / cfg_dict["phase"] / "_dataset" / cfg_dict["dataset"] / "_arm" / cfg_dict["arm"] / "_coord"
            / cfg_dict["coord"] / "_seed" / str(cfg_dict["seed"]))
...
def _leave_completed_trial(tmp_path, cfg_dict) -> None:
    ...
    with open(d / "trial_metadata.json", "w") as f:
        json.dump({"dataset": cfg_dict["dataset"], "complete": False, "killed": False, "runtime": {"trial": "3661.0"}, "progress": {"epoch": 1, "n_epochs": 35, "n_samps_seen": 200_000}, "n_crashes": {"ram": 0, "vram": 0, "other": 0}}, f)
...
def test_load_or_create_campaign_config_reuses_existing_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(cr, "load_manifold_viz_trial_config_dict", lambda: mviz_a)
    ...
    monkeypatch.setattr(cr, "load_manifold_viz_trial_config_dict", lambda: mviz_b)
...
def test_load_or_create_campaign_config_keeps_unresolved_nulls(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(cr, "load_manifold_viz_trial_config_dict", lambda: {})
...
def _stub_campaign_config(monkeypatch, hardware=None, manifold_viz=None, train_extra=None) -> dict:
...
def test_run_campaign_matrix(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(cr, "SEED0", 42)
    monkeypatch.setattr(cr, "paths", {"artifacts": tmp_path, "imgs": {}, "img_cache": tmp_path / "img_cache"})
    _stub_campaign_config(monkeypatch, train_extra={"lr": {"init": 1.0e-5}})

    scheduled = []

    def _fake_run_trial_subprocess(cfg_dict: dict, spare_render_pid=None):
        scheduled.append((cfg_dict["seed"], cfg_dict["dataset"], cfg_dict["arm"], cfg_dict["coord"],
                          cfg_dict["loss"]["loss1"]["targ"], cfg_dict["lr"]["init"]))
    ...
    _set_camp(
        monkeypatch,
        n_trials_screen=2, n_trials_refine=None, trainval=False,
        datasets=("cub", "lepid"),
        ablation_arms=[[
            {"loss.loss1.targ": "sp", "name": "sp"},
            {"loss.loss1.targ": "phylo", "name": "hp"},
        ]],
        hpo_coords=[[{"lr.init": [2.0e-5, 2.0e-6]}]],
    )
    ...
    assert set(scheduled) == {
        ...
        for coord, lr in (("LR-2e-5", 2.0e-5), ("LR-2e-6", 2.0e-6))
    }
    # seed-major, then dataset, arm, coord (the inner loop) -- the cycle order the stats levels key off
    assert scheduled[:4] == [
        (42, "cub", "sp", "LR-2e-5", "sp", 2.0e-5),
        (42, "cub", "sp", "LR-2e-6", "sp", 2.0e-6),
        (42, "cub", "hp", "LR-2e-5", "phylo", 2.0e-5),
        (42, "cub", "hp", "LR-2e-6", "phylo", 2.0e-6),
    ]
    assert scheduled[4][1] == "lepid" and scheduled[8][0] == 43

    meta = json.loads((tmp_path / "cmp_b" / "_phase" / "screen" / "phase_metadata.json").read_text())
    assert meta["matrix"] == {d: {a: ["LR-2e-5", "LR-2e-6"] for a in ("sp", "hp")} for d in ("cub", "lepid")}
...
def test_run_campaign_raises_on_arm_coord_key_collision_before_side_effects(tmp_path, monkeypatch) -> None:
    ...
    with pytest.raises(ValueError, match=r"loss1\.targ.*appear in both ablation_arms and hpo_coords"):
        _set_camp(
            monkeypatch,
            n_trials_screen=1, n_trials_refine=None, trainval=False,
            datasets=("cub",),
            ablation_arms=[[{"loss.loss1.targ": "sp", "name": "sp"}]],
            hpo_coords=[[{"loss.loss1.targ": "mp", "name": "mp"}]],
        )
...
def test_run_campaign_writes_split_overrides(tmp_path, monkeypatch) -> None:
    ...
    _set_camp(
        ...
        datasets=("cub",),
        ablation_arms=[[{"loss.loss1.targ": "sp", "name": "sp"}]],
        hpo_coords=[[{"lr.init": 2.0e-5, "name": "lr"}]],
    )
    cr.run_campaign("cmp_c", "camp")

    fpath = tmp_path / "cmp_c" / "_phase" / "screen" / "_dataset" / "cub" / "_arm" / "sp" / "_coord" / "lr" / "overrides.json"
    assert fpath.exists()
    with open(fpath) as f:
        assert json.load(f) == {"arm": {"loss.loss1.targ": "sp"}, "coord": {"lr.init": 2.0e-5}, "baseline": {}}
    # the trial itself gets the merged set
    assert scheduled == [{"loss.loss1.targ": "sp", "lr.init": 2.0e-5}]


def test_run_campaign_defers_coord_dir_until_trial_launch(tmp_path, monkeypatch) -> None:
    # a coord's dir (_dataset/<dataset>/_arm/<arm>/_coord/<coord>/, holding overrides.json) is created at
    ...
    dpath_arms = tmp_path / "cmp_defer" / "_phase" / "screen" / "_dataset" / "cub" / "_arm"
    ...
    def _fake_run_trial_subprocess(cfg_dict, spare_render_pid=None):
        assert (dpath_arms / cfg_dict["arm"] / "_coord" / cfg_dict["coord"] / "overrides.json").exists()
    ...
    assert (dpath_arms / "sp" / "_coord" / "base" / "overrides.json").exists()
...
def test_run_campaign_marks_complete_after_successful_trial(tmp_path, monkeypatch) -> None:
    ...
    dpath_trial = tmp_path / "cmp_complete" / "_phase" / "screen" / "_dataset" / "cub" / "_arm" / "sp" / "_coord" / "base" / "_seed" / "42"
    ...
    def _fake_run_trial_subprocess(cfg_dict: dict, spare_render_pid=None):
        ...
        with open(dpath_trial / "trial_metadata.json", "w") as f:
            json.dump({"dataset": "cub", "complete": False, "killed": False, "runtime": {"trial": "3661.0"}, "progress": {"epoch": 1, "n_epochs": 35, "n_samps_seen": 200_000}, "n_crashes": {"ram": 0, "vram": 0, "other": 0}}, f)
...
def _campaign_table_fpaths(dpath_phase: Path, dataset: str, arm: str, groups=_GROUPS_ON) -> list[Path]:
    dpath_dataset = dpath_phase / "_dataset" / dataset
    fpaths = []
    for group in groups:
        fpaths.append(group_path(dpath_dataset / "_arm" / arm / "arm_metrics" / "performance", group, "scores", ".png"))
        for kind in ("armcoords", "arms"):
            fpaths.append(group_path(dpath_dataset / "dataset_metrics" / kind / "performance", group, "scores", ".png"))
            fpaths.append(fpath_workbook(dpath_phase, kind, group))
    return fpaths
...
def test_run_campaign_renders_tables_at_exit(tmp_path, monkeypatch, interrupted: bool) -> None:
    ...
    for fpath in _campaign_table_fpaths(tmp_path / "cmp_render" / "_phase" / "screen", "cub", "sp"):
        assert fpath.exists(), fpath
    # ... and only for the eval groups reporting.eval has in play
    for fpath in _campaign_table_fpaths(tmp_path / "cmp_render" / "_phase" / "screen", "cub", "sp", groups=_GROUPS_OFF):
...
def test_run_campaign_del_base_eval_cache_campaign_deletes_only_at_creation(tmp_path, monkeypatch) -> None:
    ...
    _set_camp(monkeypatch, n_trials_screen=1, n_trials_refine=None, trainval=False, datasets=("cub",), ablation_arms=[[{"loss.loss1.targ": "sp", "name": "sp"}]], hpo_coords=_BASE_COORD)
...
def test_run_campaign_retries_then_fails_trial_without_progress(tmp_path, monkeypatch) -> None:
    ...
    dpath_trial = tmp_path / "cmp_fail" / "_phase" / "screen" / "_dataset" / "cub" / "_arm" / "sp" / "_coord" / "base" / "_seed" / "42"
    ...
    assert "--- Mixed" in (tmp_path / "cmp_fail" / "_phase" / "screen" / "manifest.log").read_text(encoding="utf-8")
    ...
    with open(tmp_path / "cmp_fail" / "_phase" / "screen" / "phase_metadata.json") as f:
...
def test_bump_crash_counts_reaches_coord_metadata(tmp_path) -> None:
    ...
    dpath_trial = dpath_phase / "_dataset" / "cub" / "_arm" / "sp" / "_coord" / "base" / "_seed" / "42"
...
def test_run_campaign_invalid_config_fails_at_kickoff(tmp_path, monkeypatch) -> None:
    ...
    with pytest.raises(ValueError, match="invalid config for arm 'bad' / coord 'base' on dataset 'cub'"):
        _set_camp(
            ...
            n_trials_screen=1, n_trials_refine=None, trainval=False,
            datasets=("cub",),
            ablation_arms=[[{"loss.loss1.targ": "sp", "name": "sp"}, {"loss.loss1.targ": "mp", "name": "bad"}]],
            hpo_coords=_BASE_COORD,
        )
...
def test_run_campaign_retries_recover_across_flakes_that_make_progress(tmp_path, monkeypatch) -> None:
    ...
    dpath_trial = tmp_path / "cmp_flaky" / "_phase" / "screen" / "_dataset" / "cub" / "_arm" / "sp" / "_coord" / "base" / "_seed" / "42"
    ...
    with open(tmp_path / "cmp_flaky" / "_phase" / "screen" / "phase_metadata.json") as f:
...
def test_expand_combo_groups_raises_on_duplicate_names() -> None:
    with pytest.raises(ValueError, match="Duplicate hpo_coords name"):
        cr._expand_combo_groups(
            [
                [
                    {"loss.loss1.targ": "sp", "name": "dup"},
                    {"loss.loss1.targ": "phylo", "name": "dup"},
                ]
            ],
            "hpo_coords",
        )
...
def test_expand_combo_groups_single_combo_group_unchanged() -> None:
    ...
    members = cr._expand_combo_groups(
        [
            [
                {"loss.blend.lambda": 0.3, "loss.loss2.targ": "phylo", "name": "hp"},
                {"loss.loss1.targ": "sp", "name": "sp"},
            ]
        ],
        "ablation_arms",
    ...
    assert members == [
        ("hp", {"loss.blend.lambda": 0.3, "loss.loss2.targ": "phylo"}),
...
def test_expand_combo_groups_combo_list_expands_item_and_appends_to_name() -> None:
    ...
    members = cr._expand_combo_groups(
        [
            [
                {"loss.blend.lambda": 0.3, "loss.loss2.targ": "phylo", "batch_size": [1024, 2048], "name": "hp"},
                {"loss.loss1.targ": "mp", "batch_size": [1024, 2048], "name": "mp"},
            ]
        ],
        "ablation_arms",
    )
    assert members == [
        ("hp_BS-1k", {"loss.blend.lambda": 0.3, "loss.loss2.targ": "phylo", "batch_size": 1024}),
        ("hp_BS-2k", {"loss.blend.lambda": 0.3, "loss.loss2.targ": "phylo", "batch_size": 2048}),
        ("mp_BS-1k", {"loss.loss1.targ": "mp", "batch_size": 1024}),
        ("mp_BS-2k", {"loss.loss1.targ": "mp", "batch_size": 2048}),
    ]
...
def test_expand_combo_groups_multiple_combo_lists_cross_within_item() -> None:
    ...
    members = cr._expand_combo_groups(
        [
            [
                {"loss.blend.lambda": 0.3, "batch_size": [1024, 2048], "lr.init": [7.0e-6, 2.0e-4], "name": "hp"},
            ]
        ],
        "hpo_coords",
    )
    ...
    assert dict(members)["hp_BS-2k_LR-2e-4"] == {"loss.blend.lambda": 0.3, "batch_size": 2048, "lr.init": 2.0e-4}
...
def test_expand_combo_groups_combo_list_in_unnamed_item_folds_into_derived_name() -> None:
    ...
    members = cr._expand_combo_groups([[{"loss.loss1.targ": "mp", "batch_size": [1024, 2048]}]], "ablation_arms")
    assert members == [
        ("Targ-MP_BS-1k", {"loss.loss1.targ": "mp", "batch_size": 1024}),
        ("Targ-MP_BS-2k", {"loss.loss1.targ": "mp", "batch_size": 2048}),
    ]
    coords = cr._expand_combo_groups(
        [[{"lr.init": [2.0e-6, 2.0e-5]}], [{"loss.logits.scale.init": [10.0, 100.0]}]],
        "hpo_coords",
    )
    assert [name for name, _ in coords] == [
        "LR-2e-6_Alpha-10.0", "LR-2e-6_Alpha-100.0", "LR-2e-5_Alpha-10.0", "LR-2e-5_Alpha-100.0",
    ]
    assert dict(coords)["LR-2e-5_Alpha-100.0"] == {"lr.init": 2.0e-5, "loss.logits.scale.init": 100.0}
...
def test_expand_combo_groups_derived_and_explicit_names_join_across_combo_groups() -> None:
    ...
    assert dict(members) == {
        "hp_2k": {"loss.blend.lambda": 0.3, "loss.loss2.targ": "phylo", "batch_size": 2048},
        "hp_BS-1k": {"loss.blend.lambda": 0.3, "loss.loss2.targ": "phylo", "batch_size": 1024},
        "Targ-MP_2k": {"loss.loss1.targ": "mp", "batch_size": 2048},
        "Targ-MP_BS-1k": {"loss.loss1.targ": "mp", "batch_size": 1024},
    }
...
def test_expand_combo_groups_cartesian_product_merges_and_joins_names() -> None:
    ...
    assert dict(members) == {
        "hp_2k": {"loss.blend.lambda": 0.3, "loss.loss2.targ": "phylo", "batch_size": 2048},
        "hp_1k": {"loss.blend.lambda": 0.3, "loss.loss2.targ": "phylo", "batch_size": 1024},
        "mp_2k": {"loss.loss1.targ": "mp", "batch_size": 2048},
        "mp_1k": {"loss.loss1.targ": "mp", "batch_size": 1024},
    }
    ...
    with pytest.raises(ValueError, match="ablation_arms key.*collide between combo groups"):
        cr._expand_combo_groups(
            [
                ...
                [
                    {"loss.blend.lambda": 0.4, "loss.loss2.targ": "phylo", "name": "hp4"},
                    {"loss.loss1.targ": "mp", "name": "sw2"},
                ],
            ],
            "ablation_arms",
        )
...
def test_expand_matrix_raises_on_arm_coord_key_collision() -> None:
    ...
    with pytest.raises(ValueError, match=r"batch_size.*appear in both ablation_arms and hpo_coords"):
        cr._expand_matrix(
            [[{"loss.loss1.targ": "mp", "batch_size": 2048, "name": "mp"}]],
            [[{"batch_size": [1024, 2048]}]],
        )
    arms, coords = cr._expand_matrix(
        [[{"loss.loss1.targ": "mp", "name": "mp"}, {"loss.loss1.targ": "sp", "name": "sp"}]],
        [[{"batch_size": [1024, 2048]}]],
    )
    assert arms == [("mp", {"loss.loss1.targ": "mp"}), ("sp", {"loss.loss1.targ": "sp"})]
...
def test_expand_combo_groups_null_name_skips_component() -> None:
    ...
    assert dict(members) == {
        "hp_if": {"loss.blend.lambda": 0.3, "loss.loss2.targ": "phylo", "loss.wting.cls_imb.type": "inv_freq"},
        "hp": {"loss.blend.lambda": 0.3, "loss.loss2.targ": "phylo", "loss.wting.cls_imb.type": "class_bal"},
        "mp_if": {"loss.loss1.targ": "mp", "loss.wting.cls_imb.type": "inv_freq"},
        "mp": {"loss.loss1.targ": "mp", "loss.wting.cls_imb.type": "class_bal"},
    }
    ...
    with pytest.raises(ValueError, match="at most one item per combo group may set"):
        cr._expand_combo_groups(
            [
                [
                    {"loss.loss1.targ": "mp", "name": "mp"},
                    {"loss.loss1.targ": "sp", "name": None},
                    {"loss.loss1.targ": "phylo", "name": None},
                ]
            ],
            "ablation_arms",
        )
...
def test_run_campaign_crosses_arms_with_coords(tmp_path, monkeypatch) -> None:
    ...
    def _fake_run_trial_subprocess(cfg_dict: dict, spare_render_pid=None):
        scheduled.append((cfg_dict["arm"], cfg_dict["coord"], cfg_dict["loss"]["loss1"]["targ"], cfg_dict["loss"]["sim"]))
        _leave_completed_trial(tmp_path, cfg_dict)
    ...
    _set_camp(
        ...
        n_trials_screen=1, n_trials_refine=None, trainval=False,
        datasets=("cub",),
        ablation_arms=[[{"loss.loss1.targ": "sp", "name": "sp"}, {"loss.loss1.targ": "phylo", "name": "hp"}]],
    ...
    with open(tmp_path / "cmp_groups" / "_phase" / "screen" / "_dataset" / "cub" / "_arm" / "hp" / "_coord" / "l2" / "overrides.json") as f:
        data = json.load(f)
    assert data == {"arm": {"loss.loss1.targ": "phylo"}, "coord": {"loss.sim": "l2"}, "baseline": {}}
...
def test_manifest_buckets_and_formats(tmp_path) -> None:
    ...
    def _make_trial(dataset, arm, coord, seed, complete=None, failure=None, runtime=None, epoch=0, killed=False):
        d = dpath_phase / "_dataset" / dataset / "_arm" / arm / "_coord" / coord / "_seed" / str(seed)
    ...
    _make_trial("cub", "mp", "c0", 42, complete=True, runtime="1234.5", epoch=3, killed=True)  # completed: killed
    ...
    # longest trial id) so the times line up; a killed trial (kill_thresh) is tagged
    ...
    assert text == (
        ...
        "cub/mp/c0/42 --- 0-00:20:34 --- ABANDON SHIP\n"
        "\n"
        "🏃 In Progress:\n"
        "nymph/hp/c0/42\n"
        "\n"
...
def test_manifest_completed_beats_stale_error_log(tmp_path) -> None:
    ...
    d = dpath_phase / "_dataset" / "cub" / "_arm" / "hp" / "_coord" / "c0" / "_seed" / "42"
...
def test_run_campaign_writes_manifest_tracking_outcomes(tmp_path, monkeypatch) -> None:
    ...
    dpath_phase = Path(tmp_path) / "cmp_manifest_run" / "_phase" / "screen"
...
def test_run_campaign_clears_in_progress_on_interrupt(tmp_path, monkeypatch) -> None:
    ...
    dpath_phase = Path(tmp_path) / "cmp_manifest_interrupt" / "_phase" / "screen"
...
def test_run_campaign_records_commit_hash_on_first_launch(tmp_path, monkeypatch) -> None:
    ...
    meta = json.loads((tmp_path / "cmp_commit" / "_phase" / "screen" / "phase_metadata.json").read_text())
    assert meta["commit"] == head
    # the dirty flag sits right under the hash; the test's camp yaml (config/campaigns/camp.yaml) is no tracked file, so
...
def test_run_campaign_records_campaign_and_phase_datetimes(tmp_path, monkeypatch) -> None:
    ...
    fpath_phase = tmp_path / "cmp_dt" / "_phase" / "screen" / "phase_metadata.json"
...
def test_run_campaign_relaunch_survives_duration_only_metadata_rewrite(tmp_path, monkeypatch) -> None:
    ...
    fpath_meta = tmp_path / "cmp_roundtrip" / "_phase" / "screen" / "phase_metadata.json"
...
def test_run_campaign_prunes_removed_arm(tmp_path, monkeypatch) -> None:
    ...
    dpath_arms = tmp_path / "cmp_rm_arm" / "_phase" / "screen" / "_dataset" / "cub" / "_arm"
    assert (dpath_arms / "hp" / "_coord" / "base" / "_seed" / "42" / "trial_metadata.json").exists()
    ...
    assert scheduled == []
    assert not (dpath_arms / "hp").exists()
    assert (dpath_arms / "sp" / "_coord" / "base" / "_seed" / "42" / "trial_metadata.json").exists()
    meta = json.loads((tmp_path / "cmp_rm_arm" / "_phase" / "screen" / "phase_metadata.json").read_text())
    assert meta["matrix"] == {"cub": {"sp": ["base"]}}
    text = (tmp_path / "cmp_rm_arm" / "_phase" / "screen" / "manifest.log").read_text(encoding="utf-8")
...
def test_run_campaign_prunes_removed_coord(tmp_path, monkeypatch) -> None:
    ...
    assert (dpath_coords / "loss.sim-cos" / "_seed" / "42" / "trial_metadata.json").exists()
...
def test_run_campaign_prunes_removed_dataset(tmp_path, monkeypatch) -> None:
    scheduled = _setup_completing_campaign(tmp_path, monkeypatch)
    _set_camp(monkeypatch, n_trials_screen=1, n_trials_refine=None, trainval=False, datasets=("cub", "lepid"),
              ablation_arms=[[{"loss.loss1.targ": "sp", "name": "sp"}]], hpo_coords=_BASE_COORD)
    ...
    assert scheduled == []
    dpath_datasets = tmp_path / "cmp_rm_dataset" / "_phase" / "screen" / "_dataset"
    assert not (dpath_datasets / "lepid").exists()
    assert (dpath_datasets / "cub" / "_arm" / "sp" / "_coord" / "base" / "_seed" / "42" / "trial_metadata.json").exists()
    meta = json.loads((tmp_path / "cmp_rm_dataset" / "_phase" / "screen" / "phase_metadata.json").read_text())
    assert meta["matrix"] == {"cub": {"sp": ["base"]}}
...
def test_run_campaign_prunes_removed_seed(tmp_path, monkeypatch) -> None:
    ...
    # the shrunk seed list, reselects each coord that lost a trial (its checkpoint selection and coord_metrics aggregate
    ...
    dpath_arms = tmp_path / "cmp_rm_seed" / "_phase" / "screen" / "_dataset" / "cub" / "_arm"
    assert (dpath_arms / "hp" / "_coord" / "base" / "_seed" / "43" / "trial_metadata.json").exists()
    ...
    assert scheduled == []
    for arm in ("sp", "hp"):
        assert not (dpath_arms / arm / "_coord" / "base" / "_seed" / "43").exists()
        assert (dpath_arms / arm / "_coord" / "base" / "_seed" / "42" / "trial_metadata.json").exists()
    assert reselected == [(step, dpath_arms / arm / "_coord" / "base") for arm in ("sp", "hp") for step in ("select", "stats")]
...
def test_run_campaign_drops_a_best_coord_mirror_when_a_coord_is_added(tmp_path, monkeypatch) -> None:
    # _arm/<arm>/arm_metrics/best_coords/ stands for an arm with every planned trial in: a coord added to it leaves the arm short
    ...
    scheduled = _setup_completing_campaign(tmp_path, monkeypatch)
    camp = dict(n_trials_screen=1, n_trials_refine=None, trainval=False, datasets=("cub",),
                ablation_arms=[[{"loss.loss1.targ": "sp", "name": "sp"}]])
    _set_camp(monkeypatch, **camp, hpo_coords=[[{"loss.sim": ["cos"]}]])
    cr.run_campaign("cmp_best_coord", "camp")
    dpath_best = tmp_path / "cmp_best_coord" / "_phase" / "screen" / "_dataset" / "cub" / "_arm" / "sp" / "arm_metrics" / "best_coords"
...
def test_run_campaign_use_img_cache_records_staging_runtime(tmp_path, monkeypatch) -> None:
    ...
    meta = json.loads((tmp_path / "cmp_ic_rt" / "_phase" / "screen" / "phase_metadata.json").read_text())
...
def test_run_campaign_use_img_cache_override_checked_at_startup(tmp_path, monkeypatch, side: str) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="cub"):
        _set_camp(
            ...
            n_trials_screen=1, n_trials_refine=None, trainval=False,
            datasets=("cub",),
            ablation_arms=ic if side == "arm" else [[{"loss.loss1.targ": "sp", "name": "sp"}]],
...
def _resolve_campaign(tmp_path, monkeypatch, suffix=None) -> str:
    ...
    monkeypatch.setattr(cr, "_load_campaign_config", lambda name: CampaignConfig(
        suffix=suffix, baseline_overrides={}, n_trials_screen=1, n_trials_refine=3, trainval=False, datasets=["cub"], ablation_arms=[[{"name": "s"}]], hpo_coords=_BASE_COORD,
    ))
    return cr._resolve_campaign("dev")


def test_resolve_campaign_resumes_an_existing_name(tmp_path, monkeypatch) -> None:
    # a launch under a name that already has an artifacts dir always resumes/extends that campaign
    (tmp_path / "dev").mkdir()
    assert _resolve_campaign(tmp_path, monkeypatch) == "dev"


def test_resolve_campaign_appends_the_suffix(tmp_path, monkeypatch) -> None:
    (tmp_path / "dev_foobar").mkdir()
    assert _resolve_campaign(tmp_path, monkeypatch, suffix="foobar") == "dev-foobar"


def _wire_queue(tmp_path, monkeypatch) -> Path:
    """Point the runner at a real config dir (campaign_queue.yaml + camps/ live under it) and an empty artifacts dir,
    resume campaigns by name and neutralize pytest's own argv; returns the config dir."""
    dpath_config = tmp_path / "config"
    (dpath_config / "campaigns").mkdir(parents=True)
...
def _stub_runs(monkeypatch, on_run=None, plan_changed=lambda run: False) -> list[str]:
    ...
    `plan_changed(run)` (nothing changed, by default); `on_run(campaign)` can rewrite campaign_queue.yaml mid-run to
...
def test_main_empty_queue_exits_immediately(tmp_path, monkeypatch) -> None:
    dpath_config = _wire_queue(tmp_path, monkeypatch)
    (dpath_config / "campaign_queue.yaml").write_text("campaigns:\n")  # blank list parses to None
...
_PLAN_SPEC = dict(
    n_trials_screen=1, n_trials_refine=None, trainval=False, datasets=("cub",),
    ablation_arms=[[{"loss.loss1.targ": "sp", "name": "sp"}, {"loss.loss1.targ": "phylo", "name": "hp"}]], hpo_coords=_BASE_COORD,
)
...
def test_plan_changed_compares_the_yaml_to_the_recorded_plans(tmp_path, monkeypatch, edit, expected) -> None:
    ...
    # the refine plan picks each arm's screening best (report.pick_best_coords reads real stats)
...
def test_plan_changed_stops_at_an_incomplete_earlier_phase(tmp_path, monkeypatch) -> None:
    ...
    refine phase it can't reach yet -- else the queue would relaunch it over and over."""
    _setup_completing_campaign(tmp_path, monkeypatch)
...
def test_main_runs_the_trials_added_to_a_finished_campaign_after_the_running_one(tmp_path, monkeypatch) -> None:
    ...
    def _camp(name: str, coords: list) -> CampaignConfig:
        return CampaignConfig(suffix=None, baseline_overrides={}, n_trials_screen=1, n_trials_refine=None, trainval=False, datasets=("cub",),
                              ablation_arms=[[{"loss.loss1.targ": "sp", "name": f"sp_{name}"}]], hpo_coords=coords)
...
def test_main_rejects_arguments(tmp_path, monkeypatch) -> None:
    ...
    with pytest.raises(SystemExit, match="campaign_queue"):
        cr.main()
...
def test_load_campaign_config_rejects_screen_exceeding_refine(tmp_path, monkeypatch, capsys) -> None:
    # the config layer (CampaignConfig) refuses n_trials_screen > n_trials_refine: a hard failure for the entry about
    # to run, only a warning while it sits further down the queue
    dpath_config = _wire_queue(tmp_path, monkeypatch)
    _add_camp_config(dpath_config, "a")
    _add_camp_config(dpath_config, "bad", text=_VALID_CAMP_YAML.replace("n_trials_screen: 1", "n_trials_screen: 3").replace("n_trials_refine: null", "n_trials_refine: 2"))
    with pytest.raises(ValueError, match="n_trials_screen"):
        cr._load_campaign_config("bad")
    assert cr._load_campaign_config("a").n_trials_refine is None
...
def _setup_phased_campaign(tmp_path, monkeypatch, picks: dict) -> list:
    """Wire run_campaign for a screening + refine run without real subprocesses/renders: fake trials complete
    cleanly (as _setup_completing_campaign), and the refine selection (report.pick_best_coords, which reads real
...
def test_run_campaign_refine_copies_picks_and_tops_up_seeds(tmp_path, monkeypatch) -> None:
    # after the screening matrix (2 arms x 2 coords x 1 seed) the refine phase takes each arm's pick, copies its
    # screening coord dir wholesale into refine/ (the seed-42 trial arrives complete, with overrides.json) and runs
    # the remaining seeds 43, 44 there -- seed-major, arms in campaign order -- so the refine tree reads as if
    ...
    _set_camp(
        monkeypatch,
        n_trials_screen=1, n_trials_refine=3, trainval=False, datasets=("cub",),
    ...
    assert json.loads((dpath_pick / "_seed" / "42" / "trial_metadata.json").read_text())["complete"] is True  # the copied screening trial
    assert not (dpath_pick / "_seed" / "42" / "chkpts").exists()
    for seed in ("43", "44"):
        assert json.loads((dpath_pick / "_seed" / seed / "trial_metadata.json").read_text())["complete"] is True
    ...
    assert (dpath_refine / "_dataset" / "cub" / "_arm" / "hp" / "_coord" / "loss.sim-cos" / "_seed" / "44").exists()
    ...
    # the refine phase's own stats tree renders on the way out, both kinds (phase_metrics workbooks and
    # dataset_metrics pngs): arms/ only -- each arm has its single pick there, so no armcoords/
    assert fpath_workbook(dpath_refine, "arms", "native").exists()
    assert (dpath_refine / "_dataset" / "cub" / "dataset_metrics" / "arms").exists()
    assert not (dpath_refine / "phase_metrics" / "armcoords").exists()
    assert not (dpath_refine / "_dataset" / "cub" / "dataset_metrics" / "armcoords").exists()
    # screening: both coords, still just seed 42
    dpath_screen = tmp_path / "cmp_refine" / "_phase" / "screen" / "_dataset" / "cub" / "_arm" / "sp" / "_coord"
    assert (dpath_screen / "loss.sim-cos" / "_seed" / "42").exists() and (dpath_screen / "loss.sim-geo1" / "_seed" / "42").exists()
    assert not (dpath_screen / "loss.sim-geo1" / "_seed" / "43").exists()
...
def test_run_campaign_refine_prunes_removed_seed(tmp_path, monkeypatch) -> None:
    ...
    for arm, coord in (("sp", "loss.sim-geo1"), ("hp", "loss.sim-cos")):
        ...
        assert not (dpath_pick / "_seed" / "44").exists()
        assert json.loads((dpath_pick / "_seed" / "43" / "trial_metadata.json").read_text())["complete"] is True
    ...
    assert (dpath_screen / "_dataset" / "cub" / "_arm" / "sp" / "_coord" / "loss.sim-geo1" / "_seed" / "42" / "trial_metadata.json").exists()
...
def test_run_campaign_refine_null_skips_refine(tmp_path, monkeypatch) -> None:
    ...
    assert cr.run_campaign("cmp_norefine", "camp")

    assert [t[0] for t in scheduled] == ["screen", "screen"]
    assert not (tmp_path / "cmp_norefine" / "_phase" / "refine").exists()


def test_run_campaign_refine_equal_counts_only_copies(tmp_path, monkeypatch) -> None:
    # n_trials_refine == n_trials_screen: the refine phase is the copy of each pick plus its stats, no new trials
    scheduled = _setup_phased_campaign(tmp_path, monkeypatch, {("sp", "cub"): "loss.sim-cos"})

    _set_camp(
        monkeypatch,
        n_trials_screen=1, n_trials_refine=1, trainval=False, datasets=("cub",),
                           ablation_arms=[[{"loss.loss1.targ": "sp", "name": "sp"}]], hpo_coords=_COORDS_SIM
    )
    assert cr.run_campaign("cmp_eq", "camp")

    assert [t[0] for t in scheduled] == ["screen", "screen"]
    dpath_refine = tmp_path / "cmp_eq" / "_phase" / "refine"
    assert (dpath_refine / "_dataset" / "cub" / "_arm" / "sp" / "_coord" / "loss.sim-cos" / "_seed" / "42" / "trial_metadata.json").exists()
    assert json.loads((dpath_refine / "phase_metadata.json").read_text())["seeds"] == [42]
    assert fpath_workbook(dpath_refine, "arms", "native").exists()
...
def test_run_campaign_refine_replaces_pick_on_relaunch(tmp_path, monkeypatch) -> None:
    ...
    assert json.loads((dpath_coords / "loss.sim-cos" / "_seed" / "42" / "trial_metadata.json").read_text())["complete"] is True
    # the replaced pick stays in screening: coord decisions are screening's, its trials there are kept
    assert (tmp_path / "cmp_newpick" / "_phase" / "screen" / "_dataset" / "cub" / "_arm" / "sp" / "_coord" / "loss.sim-geo1" / "_seed" / "42").exists()


def test_run_campaign_stops_after_incomplete_phase(tmp_path, monkeypatch, capsys) -> None:
    # a phase with a trial that failed for good ends the campaign there: no refine phase is started (so no pick is
...
def test_run_campaign_removed_coord_drops_its_refine_pick(tmp_path, monkeypatch) -> None:
    ...
    assert json.loads((dpath_coords_refine / "loss.sim-cos" / "_seed" / "42" / "trial_metadata.json").read_text())["complete"] is True
...
def test_run_campaign_live_edit_adds_trials_between_trials(tmp_path, monkeypatch) -> None:
    ...
    meta = json.loads((tmp_path / "cmp_live_add" / "_phase" / "screen" / "phase_metadata.json").read_text())
    assert meta["matrix"] == {"cub": {"sp": ["base"], "hp": ["base"]}}
    text = (tmp_path / "cmp_live_add" / "_phase" / "screen" / "manifest.log").read_text(encoding="utf-8")
...
def test_run_campaign_live_edit_removes_items_between_trials(tmp_path, monkeypatch) -> None:
    ...
    assert not (dpath_coords / "loss.sim-geo1").exists()
    assert (dpath_coords / "loss.sim-cos" / "_seed" / "43" / "trial_metadata.json").exists()
...
def test_run_campaign_live_edit_during_refine_screens_added_arm_first(tmp_path, monkeypatch) -> None:
    ...
    def _add_hp_during_first_refine_trial(cfg_dict, spare_render_pid=None):
        fake_ok(cfg_dict, spare_render_pid)
        if cfg_dict["phase"] == "refine" and len(scheduled) == 3:
            _set_camp(monkeypatch, n_trials_screen=1, n_trials_refine=2, trainval=False, datasets=("cub",),
                      ablation_arms=_ARMS_SP_HP, hpo_coords=_COORDS_SIM)

    monkeypatch.setattr(cr, "_run_trial_subprocess", _add_hp_during_first_refine_trial)
...
def test_run_campaign_live_edit_switches_refine_off(tmp_path, monkeypatch) -> None:
    ...
    def _switch_refine_off(cfg_dict, spare_render_pid=None):
        fake_ok(cfg_dict, spare_render_pid)
        if cfg_dict["phase"] == "refine":
            _set_camp(monkeypatch, n_trials_screen=1, n_trials_refine=None, trainval=False, datasets=("cub",),
                      ablation_arms=[[{"loss.loss1.targ": "sp", "name": "sp"}]], hpo_coords=_COORDS_SIM)
    ...
    assert (tmp_path / "cmp_live_off" / "_phase" / "refine" / "_dataset" / "cub" / "_arm" / "sp" / "_coord" / "loss.sim-geo1" / "_seed" / "43" / "trial_metadata.json").exists()
...
def test_run_campaign_live_edit_invalid_yaml_keeps_last_plan(tmp_path, monkeypatch, capsys) -> None:
    ...
    _set_camp(monkeypatch, n_trials_screen=3, n_trials_refine=None, trainval=False, datasets=("cub",),
              ablation_arms=[[{"loss.loss1.targ": "sp", "name": "sp"}]], hpo_coords=_BASE_COORD)
    fake_ok = cr._run_trial_subprocess
    arms_sp_hp = [[{"loss.loss1.targ": "sp", "name": "sp"}, {"loss.loss1.targ": "phylo", "name": "hp"}]]
...
def test_is_dirty_tracks_tracked_file_edits_and_an_untracked_camp_yaml(tmp_path, monkeypatch) -> None:
    ...
    # the campaign's own yaml, config/campaigns/<name>.yaml, which the commit must have for the run to be its code
    def git(*args):
    ...
    (tmp_path / "config" / "campaigns" / "camp.yaml").write_text("x")
    git("add", "a.py", "config/campaigns/camp.yaml")
    git("commit", "-q", "-m", "init")
    ...
    assert cr._is_dirty("camp") is False
    (tmp_path / "config" / "campaigns" / "camp_new.yaml").write_text("x")
