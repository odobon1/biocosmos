...
class EvaluationPipeline:
    ...
    def evaluate_split(self, modelw, verbose_batch_loss=False):
        ...
        n_correct        = 0
        for imgs_b, targ_classes_enc_b, texts_b, rank_keys in tqdm(self.dataloader, desc=f"Eval ({self.split_type})", leave=False):
            ...
            if B == self.batch_size:  # only compute loss for full batches
                with torch.no_grad(), autocast(device_type=modelw.device.type):
                    ...
                    loss_b      = modelw.compute_loss(logits_b, targ_classes_enc_b, rank_keys)
