...
1. Define the campaign in `config/camps/<campaign>.yaml`:
   ...
   **Phases.** A campaign runs in two phases, each with its own artifact tree under `artifacts/<campaign>/<phase>/` (below). The **screening** phase is the full matrix above. Once it has run, the **qual** phase — skipped when `n_trials_qual` is `null` — picks, for each arm on each dataset, its **best coord** from the screening results: the coord with the highest across-trial mean Native mAP composite All score (`comp.map.all` at the mAP-selected checkpoint; ties to the first coord in campaign order). The pick's whole coord dir is copied from `screening/` into `qual/` — its screening trials, `config.json` / `overrides.json` / `coord_metadata.json` and `coord_stats/` — and the qual phase then runs seeds `SEED0 + n_trials_screen .. SEED0 + n_trials_qual - 1` for it, so the qual tree reads as if `n_trials_qual` trials had run for each pick from the start (its `coord_stats/` and every table re-aggregate over all of them as the new trials complete; with `n_trials_qual == n_trials_screen` it is just the copy plus its stats). Picks accumulate in `qual/campaign_metadata.json` across launches: recorded picks are always kept (their qual trials stay), and a relaunch whose screening best has since moved — e.g. a matrix extension whose added coord now screens best for an arm — adds the new best as a further pick for that arm, copied over and topped up like any other, alongside the earlier pick(s); an arm or dataset added later starts from just its current best. Each phase's `campaign_metadata.json` records its planned **matrix** (`{dataset: {arm: [coords]}}` — every coord in screening, each arm's pick(s) in qual), which is what the stats code keys its rows and render cycles off. A phase whose trials don't all complete — a trial that failed for good (retries exhausted) — ends the campaign there: the next phase is not started (the queue moves on to the next campaign), and a relaunch resumes the failed trials before proceeding.
   ...
   Putting it together, a complete campaign (`config/camps/foobar.yaml`):
    ...
    suffix: null
    ```
   Queued as `camp.foobar` and launched via `python -m campaign_runner`, this runs **60 trials**: 3 arms (`sp`, `mp`, `hp`) × 2 coords (`LR-1.0e-5`, `LR-2.0e-5`) × 2 datasets (`cub`, `lepid`) × 5 seeds (`42`–`46` — trial seeds are `SEED0 .. SEED0 + n_trials_screen - 1`, and `SEED0 = 42`), seed-major (then dataset, arm, coord). Each trial's artifacts land under `artifacts/foobar/screening/datasets/<dataset>/arms/<arm>/coords/<coord>/<seed>/` (the qual phase's under `artifacts/foobar/qual/…`); each phase's tree:
    ```
    artifacts/<campaign>/<phase>/               screening/, then qual/ (n_trials_qual set) and trainval/ (trainval: true) -- the same tree each
      datasets/<dataset>/
        arms/<arm>/
          coords/<coord>/
            <seed>/                          one trial: evals/, learning_curves/, logs/, data_trial.pkl, trial_metadata.json (+ model.pt, no evals/, in trainval/)
        ...
        dataset_stats/
          ...
          arms/{map,acc}/<group>/            metrics.png, convergence.png  (best coord per arm)
      campaign_stats/
        arm_coords/{map,acc}/<group>.xlsx    (across all datasets x arms x coords x seeds)
        arms/{map,acc}/<group>.xlsx          (best coord per arm; across all datasets x arms x seeds)
      campaign_metadata.json (incl. the phase's planned matrix), cfg_baseline.json, manifest.log, time.pkl
    ```
   (the `*_stats/` entries exist in `screening/` and `qual/` only — the trainval phase runs no evals)
...
Evaluations run at every checkpoint threshold during training (`n_chkpts` evenly spaced thresholds, the last landing at the conclusion of training), each persisting under `artifacts/<campaign>/<phase>/datasets/<dataset>/arms/<arm>/coords/<coord>/<seed>/evals/eval<k>/` (`k` = 1..`n_chkpts`, `eval<n_chkpts>` being the final eval). Each eval writes one metrics file per eval group (`{native,native_macro,joint,joint_macro}.json`), and — when the trial's seed falls in `manif_viz.yaml`'s seed window (the manifold-viz master switch, see below) — caches its **raw** t-SNE/PCA projections to `projections.npz` (UMAP appended post-trial) and its raw embeddings to `embs.npz`, and renders its plots to `viz/`; `evals/base/` (materialized from the base-eval cache) opens the sequence.
...
**Re-rendering by hand.** `python -m tools.regen_manif_viz <campaign>` re-renders every trial in a campaign from its cached projections — no train/eval rerun — sweeping each phase's (screening and qual) `campaign_metadata.json` matrix and skipping trials that never ran. It reads the **live** `config/manif_viz.yaml`, so edits to colors, `bg_color`, `eval_duration`, `orient.ema_tau` and the `plot_*` group toggles take effect for an already-run campaign; the cached coordinates are reused, so `tsne.*` / `umap.*` changes need the coords deleted first. Pass a single trial (`<campaign>/<phase>/datasets/<dataset>/arms/<arm>/coords/<coord>/<seed>`) to scope it to one, `evo_only` / `no_evo` to render only the evolving GIFs or only the per-eval plots, and `snapshot` to use the campaign's frozen config instead of the live one — which is the form the post-trial render worker spawns. Because UMAP needs the raw embeddings, **every** viz trial caches `embs.npz` per eval (roughly `N × embed_dim × 2` bytes), not just pooled ones.
