...
When `dev.save_mid_evals: true`, each train-time eval persists its results under `artifacts/<campaign>/<setting>/<dataset>/<seed>/evals/<thresh>/`, where `<thresh>` is the checkpoint threshold that triggered the eval, formatted compactly (e.g. crossing 100,000 samples with `chkpt_every: 50_000` writes to `evals/100k/`). Each snapshot contains a `metrics.json` and a `viz/` directory of t-SNE/PCA projections, matching the layout under `chkpts/final/`. Unlike the rolling `chkpts/in_progress/` checkpoint (overwritten each threshold), these snapshots accumulate one per threshold, preserving the trajectory of eval metrics and embedding structure across training. An `evals/_base/` snapshot is also written, copied from the base-model eval cache (`base_eval_cache/.../{metrics.json,viz/}`), so the untrained-model baseline sits alongside the threshold snapshots. When `false`, no per-threshold eval results are saved.

## Manifold Visualizations
Each `viz/` directory holds t-SNE and PCA projections of the eval image embeddings under `viz/{tsne,pca}/`. For each method there are ten plots, each colored by penultimate-level group (`*_penult.png`) and by leaf class (`*_leaf.png`): ID-only (`id_*`), OOD-only (`ood_*`), the joint ID+OOD projection (`fullset_*`), and that same joint projection with one partition masked out — ID-only (`fullset_id_*`) and OOD-only (`fullset_ood_*`). The masked variants share the exact `fullset_*` geometry (the other partition's points are made transparent, not removed, so coordinates and axis limits are identical), making it easy to see where each partition falls within the shared embedding.
...
When `manifold_viz.yaml`'s `stoch_layer: true`, points are drawn in a (deterministic) shuffled order rather than class-by-class, so no single class is plotted entirely on top of the others; set `false` to draw class-grouped instead.
