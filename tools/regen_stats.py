...
of them take effect for an already-run campaign, and the phase's FROZEN train.yaml (cfg_baseline.json) for whether the
base eval is a selection candidate (dev.reporting.eval.base_chkpt_sel). Each trial's cached per-checkpoint
...
def regen_campaign(campaign, cfg_stats):
    ...
    for phase in ("_screen", "qual"):  # the trainval phase runs no evals: nothing to select, aggregate or render
        ...
        base_sel = load_json(ArtifactManager.dpath_phase / "cfg_baseline.json")["train"]["dev"]["reporting"]["eval"]["base_chkpt_sel"]
