...
def build_splits() -> None:
    ...
    rfpaths_all = np.array(
        [normalize_cub_rfpath(item[0][0]) for item in data["image_files"]],
        dtype=str,
    )

    rfpaths_pool = rfpaths_all[idxs_pool]
    rfpaths_test_id = rfpaths_all[idxs_test_id]
    rfpaths_test_ood = rfpaths_all[idxs_test_ood]

    img_ptrs, rfpath_2_skey, _ = build_img_ptrs(rfpaths_all)
    
    cid_2_n_samps = {cid: len(img_ptrs[cid]) for cid in cids}

    n_cids_all = len(cids)
    n_samps_all = sum(cid_2_n_samps.values())
